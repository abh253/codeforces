...
ll helper(vll &gap,ll t,ll s,ll p){
    ll ans=t;
    For(i,1,t){
        ll tcpy=t;
        ll ca=1;
...
void solve(){
    ...
    sort(all(v));
    vll gap(t+1); //gap[i] means gap between i and i+1;
    // gap[i]==1 if wire ends there 
    FOR(i,n){
        if(v[i][0]==1){
            gap[t]=1;
            gap[v[i][1]]=1;
    ...
    ll ans=helper(gap,t,s,p);
    reverse(gap.begin()+1,gap.end());
    ans=min(ans,helper(gap,t,s,p));
    cout<<ans<<endl;
