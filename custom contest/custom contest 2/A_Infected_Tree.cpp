...
ll dfs1(ll i,ll p,vector<vll>&t,vll&child){

    ll ans=1;
    for(auto j:t[i]){
        if(j!=p){
            ans+=dfs1(j,i,t,child);
        }
    }
    return child[i]=ans;
}

ll dfs2(ll i,ll p,vector<vll>&t,vll&child){
    vector<pll> zoro;
    ll total=0;
    for(auto j:t[i]){
        if(j!=p){
            ll sa=dfs2(j,i,t,child);
            zoro.pb({j,sa});
            total+=sa;
        }
    }
