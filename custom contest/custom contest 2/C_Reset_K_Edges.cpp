...
vll parent;
void dfs1(ll i,ll p,vector<set<ll>>&t,vector<vll>&v,ll depth=0){
    if(v.size()==depth){
        v.pb({});
    }
    v[depth].pb(i);
    for(auto j:t[i]){
        if(j!=p)
            dfs1(j,i,t,v,depth+1);
    }

}

bool check(vector<vll>&v,vector<set<ll>>t,ll mid,ll k){
    // we want maximum height to be mid;
    ll n=t.size();
    vll height(n);
    ll kreq=0;
    rep(i,v.size()-1,2){
        for(auto j:v[i]){
            // every j finds out the maximum height from their childs
            ll mx=0;
            for(auto c:t[j]){
                mx=max(mx,height[c]);
            }
