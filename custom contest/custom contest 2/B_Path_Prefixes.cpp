...
void dfs(ll i,ll p,vector<map<ll,ll>>&ai,vector<map<ll,ll>>&bi,vll &ans,vll &bees,ll ca=0,ll cb=0,ll depth=0){
    if(cb<=ca){
        ans[i]=depth;
    }else{
        ll idx=upper_bound(all(bees),ca)-bees.begin();
        ans[i]=idx;
    }
    for(auto it:ai[i]){
        if(it.ff==p)continue;
        ...
        ll b=bi[i][j];
        bees.pb(cb+b);
        dfs(j,i,ai,bi,ans,bees,ca+a,cb+b,depth+1);
        bees.pop_back();
    }
}
...
void solve(){
    ...
    // vll v(n);vinput(v);
    // vll parent(n);
    vector<map<ll,ll>>ai(n);
    vector<map<ll,ll>>bi(n);
    For(i,1,n-1){
        ll p,a,b;cin>>p>>a>>b;  
        p--;
        ai[p][i]=a;
