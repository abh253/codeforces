    ...
    for(auto it:edges){
        ...
        if(intro){
            ...
            for(int i=0;i<n;i++){
                ...
                if(mxSiz.size()>(intro+1)){
                    mxSiz.erase(mxSiz.begin());
                }
            }
            ll cursiz=0;
            for(auto it:mxSiz){
            ...
            mxSize=max(mxSize,cursiz);
        }
        else{
            ll cursiz=Size[findParent(u,parent)];
            mxSize=max(mxSize,cursiz);
        }
