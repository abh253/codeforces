...
void solve(){
    ...
    while(1){
        ll same=1;
        For(i,1,n-1){
            while(zoro[i].ff>zoro[0].ff){
                ans.pb({zoro[i].ss+1,zoro[0].ss+1});
                zoro[i].ff=(zoro[i].ff+zoro[0].ff-1)/zoro[0].ff;
            }
            if(zoro[i].ff==zoro[0].ff){
                same++;
            }
        }
        if(same==n){
...
// observation
// if i have 2 and there is no 1 then i can make all equal 
// 