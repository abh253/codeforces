...
ll check(ll a,ll c,ll z){
    // find multiple of z between a and c (inclusive)
    ll x=(a+z-1)/z;
    x*=z;
    if(x<=c)return x;
    return -1;
...
void solve(){
    ...
    // ll n;cin>>n;
    // vll v(n);vinput(v);
    ll a,b,c,d;inp4(a,b,c,d);
    // a-c 
    // b-d
    //3*4=12
    // z1 will go with x and z2 will go with y
    //multiple between a+1 to c
    // 
    ll z=a*b;
    For(i,1,sqrt(z)+1){
        if(z%i)continue;
