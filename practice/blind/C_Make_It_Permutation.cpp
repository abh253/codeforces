...
void solve(){
    ...
    vll zoro;
    for(auto it:st)zoro.pb(it);
    ll ans2=c*zoro.size()+d;
    FOR(i,zoro.size()){
        ans2=min(ans2,(zoro[i]-(i+1))*d+c*(zoro.size()-(i+1)));
    }
    cout<<ans+ans2<<endl;
...
// observations
// any number greater than n has to go 
// any repeatation has to go
// after that we fill the gaps if any otherwise array is sorted
