...
void solve(){
    ...
    vector<string>v(n);
    vinput(v);
    // if i first calulate which row and which column are giving problem
    // then a row and a column can be taken care of if i put O at (row,col)
    // solution is always possible and this is how we make sure all element 
    vll row;
    vll col;
    FOR(i,n){
        bool flag=1;
    ...
    // errA(row);
    // errA(col);
    FOR(i,min(row.size(),col.size())){
        v[row[i]][col[i]]='O';
    }
    if(row.size()>col.size()){
        For(i,col.size(),row.size()-1){
            v[row[i]][0]='O';
        }
    }else if(col.size()>row.size()){
        For(i,row.size(),col.size()-1){
            v[0][col[i]]='O';
        }
    }
    FOR(i,n){
