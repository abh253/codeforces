...
int main(){
	...
	while(testcase--){
        ...
        ll n,m1,m2;inp3(n,m1,m2);
        vector<ll> Size1(n+1, 1), Parent1(n+1, 0);
        for (ll i = 1; i < n+1; i++)
        Parent1[i] = i;
        FOR(i,m1){
            ll u,v;cin>>u>>v;
            Union(u,v,Size1,Parent1);
        }
        vector<ll> Size2(n+1, 1), Parent2(n+1, 0);
        for (ll i = 1; i < n+1; i++)
        Parent2[i] = i;
        FOR(i,m2){
            ll u,v;cin>>u>>v;
            Union(u,v,Size2,Parent2);
        }
        vector<vll>ans;

        For(i,1,n){
            For(j,i+1,n){
                if(findParent(i,Parent1)!=findParent(j,Parent1) and 
                findParent(i,Parent2)!=findParent(j,Parent2)){
                    ans.pb({i,j});
                    Union(i,j,Size1,Parent1);
                    Union(i,j,Size2,Parent2);
                }
            }
        }
        cout<<ans.size()<<endl;
        FOR(i,ans.size()){
            err2(ans[i][0],ans[i][1]);
        }
	}
}
