...
void compress(vector<pll>&v){
    if(v.size()==0)return;
    vector<pll> fin;
    // if(v.size()>0)
    fin.pb(v[0]);
    For(i,1,v.size()-1){
        if(v[i].ff<=fin.back().ss){
            pll x=fin.back();
            x.ff=min(x.ff,v[i].ff);
            x.ss=max(x.ss,v[i].ss);
            fin.pop_back();
            fin.pb(x);
        }else{
            fin.pb(v[i]);
...
void solve(){
    ...
    ll n,m;cin>>n>>m;
    // vll v(n);vinput(v);
    // we need to take care of non decreasing part first
    // the part which is decreasing and increasing =increasing
    // the part which is decreasing and none =decreasing
    // which is give increasing make that part increasing
    // which not needed to be increasing make that decreasing
    ...
    FOR(i,incseg.size()){
        // cout<<"HEL"<<endl;
        For(j,incseg[i].ff,incseg[i].ss){
            ans[j]=1000-incseg[i].ff+1;
        }
    }
    For(i,1,n){
        if(ans[i]==0){
    ...
    FOR(i,decseg.size()){
        bool flag=0;
        For(j,decseg[i].ff,decseg[i].ss-1){
