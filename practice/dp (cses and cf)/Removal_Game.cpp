...
ll solve(ll si,ll ei,vll &v ,vector<vll>&dp){
    if(si>ei)return 0;
    if(si==ei)return v[si];
    if(dp[si][ei]!=-1)return dp[si][ei];

    return dp[si][ei]=max(v[si]+min(solve(si+2,ei,v,dp),solve(si+1,ei-1,v,dp)),
                    v[ei]+min(solve(si+1,ei-1,v,dp),solve(si,ei-2,v,dp)));
}
int main(){
