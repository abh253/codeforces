...
vector<string>z={"1110111", "0010010", "1011101", "1011011", "0111010", "1101011", "1101111", "1010010", "1111111", "1111011"};
um<string,int>mp;

string solve(ll idx,vector<string>&v,ll k,vector<vector<string>>&dp){
    ...
    FOR(i,1<<cnt){
        ...
        if(mp.count(s)){
            string a=solve(idx+1,v,k-curk,dp);
            
            if(a.length()==v.size()-idx-1){
                a=to_string(mp[s])+a;
                cur=max(cur,a);
            }
        }
