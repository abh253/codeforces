...
ll n;
ll solve(ll cn,ll sum,vector<vll>&dp){
    if(sum==0){
        return 1;
    }
    if(cn>n or sum<0)return 0;

    if(dp[cn][sum]!=-1){
        return dp[cn][sum];
    }
    dp[cn][sum]=solve(cn+1,sum-cn,dp)%mod+solve(cn+1,sum,dp)%mod;
    dp[cn][sum]%=mod;
    return dp[cn][sum];
}
int main(){
	...
	while(testcase--){
        ...
        if(num&1){
            ...
            vector<vll>dp(n+1,vll(num+1,-1));
            cout<<(solve(1,num/2,dp)*power(2,mod-2,mod))%mod<<endl;
