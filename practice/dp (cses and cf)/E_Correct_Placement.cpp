...
void solve(){
    ...
    for(int i=0;i<n;i++){
        while(si<n and zoro[si][0]<v[i][0]){
            s2.push({zoro[si][1],zoro[si][2]});
            si++;
        }

        while(!s2.empty() and s2.top().ff>=v[i][1]){
            s2.pop();
        }

        while(!s1.empty() and s1.top().ff>=v[i][1]){
            s1.pop();
        }
...
// how will i do this
// i need index(j) which has (xj<xi and yj<yi) or (xj<yi and yj<xi)
// 
//observation
// for every x i want to know min y it have 
// sort the vector (x increasing and for same x decreasing)
// then
// keep a stack with value of y for current index i all visited have smaller x that can 