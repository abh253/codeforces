...
ll solve(ll x,ll idx,vint &coin,vector<vector<int>>&dp){
    if(x==0){
        return 1;
    }
    if(idx==coin.size()){
    ...
    ll a=0;
    if(x-coin[idx]>=0){
        a+=solve(x-coin[idx],idx,coin,dp);
        a%=mod;
    }
    a+=solve(x,idx+1,coin,dp);
    a%=mod;
    return dp[x][idx]=a;
...
int main(){
    ...
    ll n,x;cin>>n>>x;
    vint coin(n);vinput(coin);
    // errA(coin);
    vector<vector<int>>dp(x+1,vector<int>(n+1,-1));
    cout<<solve(x,0,coin,dp)<<endl;
