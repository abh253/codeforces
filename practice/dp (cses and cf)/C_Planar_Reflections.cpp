...
ll slave(ll i,ll j){
    ...
    return dp[i][j]=(slave(i-1,j)+slave(n-i,j-1))%mod;
}
...
//     return dp[i][j]=(slave(i-1,j)%mod+slave(n-i,j-1)%mod)%mod;
// }
    // dp[i][j]=number of particles when i planes left and decay rate is j
// i get it why its wrong because when x planes left direction is important 
// so i have to find n-i plannes left moving in left direction is conguent to what 