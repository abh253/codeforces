...
int main(){
	...
	while(testcase--){
        ...
        ll ans=max(v[0],x*v[0]);

        For(i,1,n-1){
            dp[i][0]=max(max(0LL,v[i]),v[i]+dp[i-1][0]);
            dp[i][1]=max(x*v[i],max(v[i]*x+dp[i-1][1],v[i]*x+dp[i-1][0]));
            dp[i][2]=max(v[i],v[i]+max(dp[i-1][1],max(dp[i-1][2],dp[i-1][0]))); 

            ans=max(max(dp[i][0],max(dp[i][1],dp[i][2])),ans);
        }
        ans=max(0LL,ans);
