...
// ll solve(ll n,ll lus,vector<vint>&dp){
//     if(n==0){
//         return 1;
//     }
//     if(dp[n][lus]!=-1){return dp[n][lus];}
//     ll ans=0;
//     if(lus==0){
//         ll a=solve(n-1,0,dp);
//         ll b=solve(n-1,1,dp);
...
int main(){
	...
	ll testcase;
	cin>>testcase;
        vector<vll>dp(1000001,vll(2,0));
        // cout<<(solve(n-1,0,dp)+solve(n-1,1,dp))%mod<<endl;
        dp[0][0]=1;
        dp[0][1]=1;
        For(i,1,1000000){
            For(j,0,1){
                if(j==0)
                dp[i][j]=4*dp[i-1][0]+dp[i-1][1];
                else
                dp[i][j]=2*dp[i-1][1]+dp[i-1][0];
