...
void solve(){
    ...
    // total operations required in a subarray of length l=>l-1-gaps , where gaps denotes gap between partitions
    ll ans=n*(n+1)*(n-1)/6;

    // need to subtract gaps from the ans
    
    // gap is define as an index k such that maximum(l,k-1) < min(k,r);here l and r  are boundary

    FOR(i,n){// setting the middle which lies left of the gap
        
        vll mnv(n,INT_MAX);

        For(j,i+1,n-1){
            mnv[j]=min(mnv[j-1],v[j]);
        }        
        ll k=n-1;
        ll mx=v[i];
