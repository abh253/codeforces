...
void solve(){
    ...
    for(int i=0;i<n;i++){
        if(sum%2==0){
            if(s.count(v[i]+sum/2))
                paircnt++;
            else if(!s.count(v[i]-sum/2)){
                alonecnt++;
            }
        }else{
            alonecnt++;
    ...
    // err2(paircnt,alonecnt);
    ll ans=0;
    For(i,0,paircnt){
        ll sa=(nCr(paircnt,i)*nCr(m,i))%mod;
        sa=(sa*fact[i])%mod;
        sa=(sa*power(m-i,alonecnt,mod))%mod;
        sa=(sa*power(((m-i)*(m-i-1))%mod,paircnt-i,mod))%mod;
