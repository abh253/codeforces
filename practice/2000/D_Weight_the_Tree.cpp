...
void dfs(ll i,ll p){
    ...
    for(auto j:t[i]){
        ...
        if(dp[0][j]>=dp[1][j]){
            ...
            not_take.ff+=dp[0][j].ff;
            not_take.ss+=dp[0][j].ss;

        }else{
            not_take.ff+=dp[1][j].ff;
            not_take.ss+=dp[1][j].ss;
        }
...
void assignment(ll i,ll p,bool bad){
    bool f=true;
    if(bad or dp[0][i]>dp[1][i]){
        good[i]=0;
        f=false;
    }else{
        good[i]=1;
...
void solve(){
    ...
    dp=vector<vector<pll>>(2,vector<pll>(n,{0,0}));

    dfs(0,-1);

    good=vll(n);
    assignment(0,-1,false);
    vll a(n);
    ll sum=0;
    ll ans=0;
    FOR(i,n){
