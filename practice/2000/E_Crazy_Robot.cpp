...
void solve(){
    ...
    FOR(i,n){
        FOR(j,m)
            if(v[i][j]=='L'){lab={i,j};break;}
    }

    // vector<vint>visited(n,vint(m));
    ...
    for(int i=0;i<4;i++){
        ll a=lab.ff+dir[i];
        ll b=lab.ss+dir[i+1];
        // visited[lab.ff][lab.ss]=1;
        if((a>=0 and a<n and b>=0 and b<m and v[a][b]=='.')){
            q.push({a,b});
        }
    }
    ...
    while(!q.empty()){
        ll siz=q.size();
        while(siz-->0){
            pll temp=q.front();q.pop();
            ll x=temp.ff,y=temp.ss;
            // cout<<x<<" "<<y<<endl;
            // visited[x][y]=1;
            ll blocked=0;
            ll tolab=0;
