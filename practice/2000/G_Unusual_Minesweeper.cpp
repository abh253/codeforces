...
void solve(){
    ...
    FOR(i,n){
        ...
        v[i][0]+=1e9;
        v[i][1]+=1e9;
        timer[{v[i][0],v[i][1]}]=v[i][2];

        xcord[v[i][0]].insert(v[i][1]);
        ycord[v[i][1]].insert(v[i][0]);
    }
    ...
    FOR(i,n){
        ...
        ll curtime=v[i][2];
        if(!xcord[v[i][0]].count(v[i][1]))continue;
        queue<pll>q;
        q.push({v[i][0],v[i][1]});
        xcord[v[i][0]].erase(v[i][1]);
        ycord[v[i][1]].erase(v[i][0]);
        ...
        while(!q.empty()){
            ...
            // xcord y-k and y+k
            auto it =xcord[x].lower_bound(y-k);

            if(it!=xcord[x].end()){

                while(it!=xcord[x].end() and (*it <=y+k ) ){
                    ll cury=*it;
                    auto newit=it;newit++;
                    curtime=min(curtime,timer[{x,cury}]);
                    xcord[x].erase(it);
                    ycord[cury].erase(x);
                    it=newit;
                    q.push({x,cury});
            ...
            it=ycord[y].lower_bound(x-k);

            if(it!=ycord[y].end()){
                while(it!=ycord[y].end() and (*it <=x+k ) ){
                    ll curx=*it;
                    auto newit=it;newit++;
                    curtime=min(curtime,timer[{curx,y}]);
                    ycord[y].erase(it);
                    xcord[curx].erase(y);
                    it=newit;
                    q.push({curx,y});
        ...
        timerblocks.pb(curtime);
    }

    sort(all(timerblocks));
    // errA(timerblocks)
    reverse(all(timerblocks));

    FOR(i,timerblocks.size()-1){
        if(timerblocks[i+1]<=i){
            cout<<i<<endl;
            return;
        }
    }

    cout<<timerblocks.size()-1<<endl;
