...
void solve(){
    ...
    FOR(w,q){
        ...
        sort(all(v));
        ll setLca=lca(v[0][1],v[1][1]);
        FOR(j,k){
            setLca=lca(setLca,v[j][1]);
        }

        ll leftver=v[k-1][1];
        ll rightver=-1;
        FOR(j,k){
            if(lca(leftver,v[j][1])!=v[j][1]){
                if(rightver==-1){
                    rightver=v[j][1];
                }else{
                    if(depth[v[j][1]]>depth[rightver]){
                        rightver=v[j][1];
                    }
                }
            }
        ...
        ll lrdis=distance_vertex(leftver,rightver);
        bool ans=true;
        FOR(j,k){
            ll dist1=distance_vertex(leftver,v[j][1]);
            ll dist2=distance_vertex(rightver,v[j][1]);

            if((dist1+dist2)!=lrdis){
                ans=false;
            }
        }
        // err3(leftver,rightver,setLca);
        if(ans){
            // cout<<3<<endl;
            yess;
        }else{
