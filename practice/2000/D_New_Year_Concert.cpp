...
ll build(int v,int tl,int tr,vll &t,vll &a){
    ...
    return t[v]=__gcd(build(2*v+1,tl,tm,t,a),build(2*v+2,tm+1,tr,t,a));
...
ll sum(int v,int l,int r,int tl,int tr,vll &t){
    ...
    return __gcd(sum(2*v+1,l,min(mid,r),tl,mid,t),sum(2*v+2,max(mid+1,l),r,mid+1,tr,t));
}
