...
void solve(){
    ...
    vector<ll> Size2(N, 1), Parent2(N, 0);
    for (ll i = 0; i < N; i++)Parent2[i] = i;
    ...
    FOR(i,m){
        ...
        if(Union(x,y,Size,Parent)){
            if(Union(x,y,Size2,Parent2)){
                annoy=i;
            }else{
                ans[i]='1';
            }
    ...
    if(annoy!=-1){
        ...
        Union(edges[annoy][0],edges[annoy][1],Size,Parent);
        ans=string(m,'0');
