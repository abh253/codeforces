...
void solve(){
    ...
    ll n;cin>>n;
    // vll v(n);vinput(v);
    vector<vll>v(n,vll(4));// {l,r,c,index}
    FOR(i,n){
        ll l,r,c,idx;
    ...
    FOR(i,n){
        for(auto it:mp){
            if(it.ff!=v[i][2]){
                if(v[i][0]>it.ss){
                    ans[v[i][3]]=min(ans[v[i][3]],abs(v[i][0]-it.ss));
                }else{
                    ans[v[i][3]]=0;
                }
            }
        ...
        if(mp.count(v[i][2])){
            s.erase({mp[v[i][2]],v[i][2]});
            mp[v[i][2]]=max(mp[v[i][2]],v[i][1]);
            s.insert({mp[v[i][2]],v[i][2]});
        }else{
            mp[v[i][2]]=v[i][1];
        ...
        if(mp.size()>2){
            // remove the key value with least value
            pll x=*s.begin();
            s.erase(s.begin());
            mp.erase(x.ss);
        }
    ...
    rep(i,n-1,0){
        for(auto it:mp){
            ...
            if(it.ff!=v[i][2]){
                if(v[i][1]<it.ss){
                    ans[v[i][3]]=min(ans[v[i][3]],abs(v[i][1]-it.ss));
                }else{
                    ans[v[i][3]]=0;
                }
            }
        ...
        if(mp.count(v[i][2])){
            s.erase({-mp[v[i][2]],v[i][2]});
            mp[v[i][2]]=min(mp[v[i][2]],v[i][0]);
            s.insert({-mp[v[i][2]],v[i][2]});
        }else{
            mp[v[i][2]]=v[i][0];
            s.insert({-v[i][0],v[i][2]});
        }
