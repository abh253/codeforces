...
void solve(){
    ...
    // take the initial picture that has maximum location to apply type1 operation let it be t1count
    // sort then in descending order of the t1count and then 
    // cout<<"HELLO"<<endl;
    For(i,0,k){
        For(j,0,n-1){
            cin>>v[i][j];
    ...
    For(i,0,k){
        ...
        For(j,1,n-2){
            For(k,1,m-2){
                if(v[i][j][k]=='1' && v[i][j-1][k]=='0' && v[i][j][k-1]=='0'&& v[i][j+1][k]=='0'&& v[i][j][k+1]=='0'){
                    ccount++;
                }else if(v[i][j][k]=='0' && v[i][j-1][k]=='1'&& v[i][j][k-1]=='1'&& v[i][j+1][k]=='1'&& v[i][j][k+1]=='1'){
                    ccount++;
                }
            }
        }
    ...
    sort(series.rbegin(),series.rend());
    cout<<series[0][1]+1<<endl;
    vector<vll>ans;
    ll cur=series[0][1];
    For(i,1,k){
        ...
        For(j,1,n-2){
            For(k,1,m-2){
                if(v[cur][j][k]!=v[series[i][1]][j][k]){
                    ans.pb({1,j+1,k+1});
                }
            }
        }
        cur=series[i][1];
        ans.pb({2,series[i][1]+1});
    }
