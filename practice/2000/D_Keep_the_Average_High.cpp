...
void solve(){
    ...
    marked[0]=false;
    For(i,1,n){
        if(i>1 and v[i]+v[i-1]<0 and marked[i-1]){
            marked[i]=false;
        }
        if(i>2 and v[i]+v[i-1]+v[i-2]<0 and marked[i-1]==true and marked[i-2]==true ){
            marked[i]=false;
        }
    }
    ll ans=count(all(marked),true);
    cout<<ans<<endl;
...
// its easy to see that greedily picking works
// why are we concerned about 2 and 3 sized sectin
// because of this case -1 1 -1 , in this case -1,1 and 1, -1 is ok but as whole they are <0
// why subarray size more than 3 is not of any use. 
// becaseu x ,-y ,y , z if z is less than -y than one of y and z will be removed.
// so there is no need of cheking beyond size 3
