...
void solve(){
    ...
    For(i,0,n-1){
        l=min(p[i],l);
        r=max(p[i],r);
        if(i+1<n and p[i+1]>=l and p[i+1]<=r){
            continue;
        }
        ll len=r-l+1;
        ll mxlen=2*(i+1);

       if(mxlen-len < 0)continue;

        ll d=mxlen-len;
        ll left=max(0LL,l-d);
        ll right=min(n-1,r+d);
      
        if(i+1<n and p[i+1]<l){
            left=max(p[i+1]+1,left);
        }
        if(i+1<n and p[i+1]>r){
            right=min(p[i+1]-1,right);
        }
