...
void check(int i,int j,vector<vll>&v,vector<vll>&used,queue<pll>&q,vector<vll>&ans){
	if(i<0 or j<0 or i+1>=v.size() or j+1>=v[0].size())return;
	// if(used[i][j])return;
...
void solve(){
	...
	while(!q.empty()){
		...
		For(a,-1,1){
			For(b,-1,1){
				check(x.ff+a,x.ss+b,v,used,q,ans);
			}
		}
	}
	...
	FOR(i,ans.size()){
		// errA(ans[i]);
		ll idx=ans.size()-i-1;
		err3(ans[idx][0]+1,ans[idx][1]+1,ans[idx][2]);
	}
