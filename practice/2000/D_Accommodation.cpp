...
ll helper1(string &s){
    // a binary string would be given need to return maximum number of occupied apartment.
    ll m=s.length();
    ll ans=0;
    ll tap=m/4;
    ll oap=m/2;
    // T0 is two bedroom apartment with 0 lights on
    // T1 is two bedroom apartment with 1 light on
    // T2 is two bedroom apartment with 2 lights on
    // we want to decrease T2 
    // T0+T1+T2=tap
    // so to minimize T2 , we need to maximize T0 and T1
    ...
    // now count the number of ones
    ll ones=0;
    FOR(i,m){if(s[i]=='1')ones++;}
    if(dp[m]>=tap){
        ans=ones;
    }else{
        ll t2=tap-dp[m];
