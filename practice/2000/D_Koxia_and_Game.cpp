...
bool Union(ll u, ll v, vector<ll> &Size, vector<ll> &parent,vll&Cycle,vll&Selfloop)
{
...
void solve(){
    ...
    // make edges ai--bi and any component
    // if any component have multiple cycles or self loops then 0 possibilities
    // otherwise 2 ways and incase of selfloop n ways
    ll N=n;
    vector<ll> Size(N, 1), Parent(N, 0),Cycle(N,0),Selfloop(N,0);
    for (ll i = 0; i < N; i++)
        Parent[i] = i;
    
    FOR(i,n){
        if(a[i]==b[i]){
            Selfloop[findParent(a[i],Parent)]++;
        }
        else
        Union(a[i],b[i],Size,Parent,Cycle,Selfloop);
    }
    ll ans=1;
    vll visited(n);
    FOR(i,n){
        if(!visited[findParent(i,Parent)]){
            if(Cycle[findParent(i,Parent)]+Selfloop[findParent(i,Parent)] !=1){
                ans=0;
            }else{
                if(Cycle[findParent(i,Parent)])ans*=2;
                else ans*=n;
            }
            visited[findParent(i,Parent)]=1;
        }
        ans%=mod;
    }
    cout<<ans<<endl;
