...
struct compare{
    ...
    bool operator() (const pll & left, ll right)
    {
        return left.ff < right;
    }
    bool operator() (ll left, const pll & right)
    {
        return left < right.ff;
    }
};
int main(){
	...
	while(testcase--){
		ll n,m;cin>>n>>m;
        vll a(n),b(n),c(m);vinput(a);vinput(b);vinput(c);
        ...
        FOR(i,m){
            ...
            if(need.count(c[i]) and need[c[i]].size()){
                ans[i]=need[c[i]].back()+1;
                noneed[c[i]].pb(ans[i]-1);
                need[c[i]].pop_back();
                
            }else{
             if(noneed.count(c[i]) and noneed[c[i]].size()){
                 ans[i]=noneed[c[i]].back()+1;
             }   
            }
        }
        FOR(i,m){
            if(ans[i]==0)ans[i]=ans[m-1];
        }
        bool flag=1;
        for(auto it:ans)if(!it)flag=0;

        for(auto it:need){
            if(it.ss.size())flag=0;
        }
        if(flag)
        {   yess;
            errA(ans);
