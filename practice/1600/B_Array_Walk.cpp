...
//     //index,z,cursteps,bool flag
// ll solve(ll i,ll z,ll stp,bool flag,vll &v){
//     if(stp==k ){
//         return v[i];
//     }
...
//     if(z){
//         if(flag){
//             return dp[i][stp-i][z]=v[i]+max(solve(i-1,z-1,stp+1,!flag,v),solve(i+1,z,stp+1,flag,v));
//         }else{
//             return dp[i][stp-i][z]=v[i]+solve(i+1,z,stp+1,!flag,v);
//         }
//     }else{
//         return dp[i][stp-i][z]=v[i]+solve(i+1,z,stp+1,flag,v);
//     }
...
int main(){
	...
	while(testcase--){
		ll n,z;inp3(n,k,z);
        vll v(n);vinput(v);
        // dp=vector<vector<vint>>(n,vector<vint>(11,vint(6,-1)));
        // err3(n,k,z);
        // cout<<solve(0,z,0,0,v)<<endl;
        vll pref(n+1);
        For(i,1,n){
            pref[i]=pref[i-1]+v[i-1];
        }
        ll ans=accumulate(v.begin(),v.begin()+k+1,0LL);
        For(i,0,z){
            if(k-2*i+1<=0)continue;
            ll a=pref[k-2*i+1];
            ll mpr=pref[2];
            For(j,2,min(k-2*i+2,n)){
                mpr=max(mpr,pref[j]-pref[j-2]);
            }
            ans=max(ans,a+mpr*i);
            // cout<<a+mpr*z<<endl;
        }
