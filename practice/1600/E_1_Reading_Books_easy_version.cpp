...
void solve(){
    ...
    while(alike<k and blike<k){
        ...
        if(both.empty()){
            ans+=abs(Alice.top()+Bob.top());
            Alice.pop();Bob.pop();
        }else{
            if(Alice.empty() or Bob.empty() or abs(both.top())<=abs(Alice.top()+Bob.top())){
                ans+=abs(both.top());both.pop();
            } else{
                ans+=abs(Alice.top()+Bob.top());
                Alice.pop();Bob.pop();
            }
...
// divide book in three parts 
// alice like , bob like and both like 
// considering book neither one like will make answer larger and wont help at all
// the check if taking book with both like have time less than t(aliceLike) +t(boblike) then add that both liked book 
// otherwise two books