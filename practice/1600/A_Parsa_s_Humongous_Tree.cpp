...
vector<vll>lr;
vector<vll>dp;
ll dfs(ll i,int p,bool m,vector<vll>&t){
    if(dp[i][m]!=-1){
        return dp[i][m];
    }
    ll ans=0;
    ...
    for(auto j:t[i]){
        if(j!=p)
        ans+=max(abs(cur-lr[j][0])+dfs(j,i,0,t),abs(cur-lr[j][1])+dfs(j,i,1,t));
    }   
    return dp[i][m]=ans;
}
...
void solve(){
    ...
    ll ans=max(dfs(0,-1,0,t),dfs(0,-1,1,t));
    cout<<ans<<endl;
}
int main(){
...
// value should be at one of the extremes