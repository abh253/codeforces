...
ll solve2(vll &zoro){
    ...
    while(!pq.empty()){
        ...
        while(!pq.empty() and pq.top()-time <= 0){pq.pop();}
    } 
    return time;
}
...
void solve(){
    ...
    // ll x=0;
    // its benificial to mark the set with maximum childs
    // 1 3 2 1 4 5
    multiset<pll>st;
    vll zoro;
    For(i,0,n-1){
        if(ccount[i])zoro.pb(ccount[i]);
    }
    zoro.pb(1);
    sort(all(zoro),greater<ll>());
    ...
    For(i,0,zn-1){
        zoro[i]=max(0LL,zoro[i]-(zn-i));
        ans++;
    }
    sort(all(zoro),greater<ll>());

    while(!zoro.empty() and zoro.back()<=0)zoro.pop_back();
    ans+=solve2(zoro);
