...
int maxEnvelopes(vector<vector<int>>& env){
        ...
        for(int i=1;i<n;i++){
            ...
            if(env[i][1]>lis.back()){
                if(env[i][0]>env[lidx][0]){
                    lis.push_back(env[i][1]);lidx=i;
                }
            }else{
                    int x=upper_bound(lis.begin(),lis.end(),env[i][1])-lis.begin();
                    if(x!=lis.size() ){
                        lis[x]=env[i][1];
                        if(x==lis.size()-1)lidx=i;
                    }
            }
            errA(lis);
...
int main(){
    ...
    testcase=1;
	while(testcase--){
        vector<vector<int>>v={{30,50},{12,2},{3,4},{12,15}};
        maxEnvelopes(v);
	}
}
