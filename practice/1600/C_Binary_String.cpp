...
void solve(){
    ...
    For(i,1,n){
        ...
        while(l<=r){
            ...
            ll zerocnt=p[mid]-p[i-1];
            ll onecnt=totalone-((mid-i+1)-zerocnt);
            curans=min(curans,max(onecnt,zerocnt));
...
// i can do kind of binary search 
// like start i belongs to 0,n-1
// then start binary search
// if 0 in array is greater than 0, 1 in suffix
// r=mid-1
// else l=mid+1
// ans=max(0count,1count);
