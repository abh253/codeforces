...
// vector<vint>dp;
// ll solve(ll idx,ll x,vector<ll>&v){
//     if(idx==v.size()-1){
//         return 0;
//     }
...
//         if(x<=v[idx+1]){
//             ll smallans=solve(idx+1,v[idx],v);
            
//             if(smallans==INT_MAX)return dp[idx][x]=smallans;

//             return dp[idx][x]=1+smallans;
//         }else{
//             return dp[idx][x]=INT_MAX;
//         }
...
int main(){
	...
	cin>>testcase;
	while(testcase--){
		ll n,x;cin>>n>>x;vll v(n);vinput(v);
        // dp=vector<vint>(n,vint(x+1,-1));
        // ll ans=solve(0,x,v);
