...
int main(){
	...
	while(testcase--){
        ...
        while(i<=j){
            ...
            if(curitem<v[j].ff and curitem+v[j].ss>v[i].ff){
                // j++;
                // curprice+=(v[i].ss+curitem-v[i].ff);
                // curprice+=(v[i].ff-curitem)*2;
                // curitem+=v[i].ss;
                ll ex=v[i].ff-curitem;
                curprice+=(ex)*2;
                v[j].ss-=ex;
                curitem+=ex;
                j++;
            } 
            else if(curitem>=v[j].ff){
                curprice+=v[j].ss;
                curitem+=v[j].ss;
            }
            else{
