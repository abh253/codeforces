...
ll C(ll n,ll r){
    ...
    return (fact[n]*((inverse(fact[n-r],mod)*inverse(fact[r],mod))%mod))%mod;
}
...
void solve(){
    ...
    vll endpoints;
    for(auto x :active)endpoints.pb(x.ff);
    ll ans=0;
    ll cur=0;
    For(i,0,endpoints.size()-2){
        cur+=active[endpoints[i]];
        ll siz=(endpoints[i+1]-endpoints[i])%mod;
        ans=(ans+(C(cur,k)*siz)%mod)%mod;
    }
...
// i need to know how many lamps are active before the first lamps stops
// and my solution is pretty much it