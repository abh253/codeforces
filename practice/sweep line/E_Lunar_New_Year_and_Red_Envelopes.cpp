...
void solve(){
    ...
    ll n,m,k;cin>>n>>m>>k;
    // vll v(n);vinput(v);
    // dp[i][j]=min coins at time 0 to i after aftter j skips
    // base case - dp[0][0]=0; // coins at time 0 with 0 skips 
    //  j<=i;
    ...
    // if taken
    // dp[nextime][j]=min(dp[nextime][j],dp[i][j]+coins[i]);


    vector<vector<vll>>events(n+2);
    ...
    vector<vll>dp(n+1,vll(m+2,1e14));
    ...
    For(i,0,n-1){
        For(j,0,m){
            ...
            if(coins[i+1]==0){
                ...
                dp[i+1][j+1]=min(dp[i+1][j+1],dp[i][j]);
                dp[nextime[i+1]][j]=min(dp[i][j]+coins[i+1],dp[nextime[i+1]][j]);
