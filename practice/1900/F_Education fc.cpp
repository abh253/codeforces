...
void solve(){
    ...
    //binary search
    // first i will select a number
    // then will check if i can make c tugs
    // i will check it greedily if i can make this on x position no need to move
    // otherwise i will move forward
    // if even with moving forward i cant achieve that then its impossible
    ...
    while(l<r){
        ...
        for(int i=0;i<n;i++){
            ...
            if(i==n-1)break;
            //it is the number of days it took to move forward
            ll dd=(b[i]-cur-1+a[i])/a[i];

            // ll dd=((b[i]-cur-1)/a[i]) +1;
