    ...
    while(l<r){
        ...
        FOR(i,n){
            FOR(j,m){
                ...
                if(d1[0]>=t1 and d1.back()<=t2 and d2[0]>=t3 and d2.back()<=t4){
                    flag=1;
                    ans={i,j};
                }
            }
...
// obs
// bfs approach not working
// i am thinking , somehow go to all the vertices and find out the maximum distance to a black cell 
// i am thinking of binary search
//  to range 0 to 10^6
//  then check if all black squares lie in the required part
//  otherwise