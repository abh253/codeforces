...
ll n;
void leftArray(vll&left,vll&v,ll small=1){
    stack<ll>s;
    FOR(i,n){
        if(small)
...
void rightArray(vll&right,vll&v,ll small=1){
    stack<ll>s;
    rep(i,n-1,0){
        if(small)
            while(!s.empty() and v[s.top()]>=v[i]){
...
void solve(){
    ...
    vll leftSmaller(n,-1);
    vll leftGreater(n,-1);
    vll rightSmaller(n,n);
    vll rightGreater(n,n);
    leftArray(leftSmaller,v);
    leftArray(leftGreater,v,0);
    rightArray(rightSmaller,v);
    rightArray(rightGreater,v,0);

    ll ans=0;

    FOR(i,n){
        ll x=v[i]*(i-leftSmaller[i])*(rightSmaller[i]-i);
        ll y=v[i]*(i-leftGreater[i])*(rightGreater[i]-i);
        ans+=y-x;
    }
    cout<<ans<<endl;
