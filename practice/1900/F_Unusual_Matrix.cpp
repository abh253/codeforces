...
void solve(){
    ...
    ll n;cin>>n;
    // vll v(n);vinput(v);
    vector<string>a(n);vinput(a);
    vector<string>b(n);vinput(b);
    
    // order does not matter only row and column matter and if we have to do a 
    // row or col more than once then output no because its not benificial
