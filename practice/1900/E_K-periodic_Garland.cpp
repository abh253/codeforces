...
void solve(){
    ...
    For(i,1,n){
        For(j,0,2){
            if(j==0){
                dp[i][j]=(s[i-1]=='0'?0:1)+dp[i-1][0];
            }else if(j==1){
                if(i-k>=0)
                dp[i][j]=(s[i-1]=='1'?0:1)+min(dp[i-k][1]+p[i-1]-p[i-k],dp[i-1][0]);
                else
                dp[i][j]=(s[i-1]=='1'?0:1)+dp[i-1][0];
            }else{
                dp[i][j]=(s[i-1]=='0'?0:1)+min(dp[i-1][1],dp[i-1][2]);
            }
        }
    }
    ll ans;
...
// positions can be seen as i%k;
// and when we talk about all the ones at i%k then then we will iterate through first one to last one
// and when we have index of two consecutive 1
//dp states
// dp[i][0]->all 0 from start to i
// dp[i][1]-> current is 1 ,which is followed by 0 or many consicutive 0s
// dp[i][2]->all zeroes after periodic ones
