...
void solve(){
    ...
    ll dp[cnt.size()+1][n+1];
    memset(dp,sizeof(dp),0);

    dp[0][0]=1;

    //dp[i][j] means i elements and j of them have been choosen form prime part

    for(ll i=1;i<cnt.size()+1;i++){
        for(ll j=0;j<=n;j++){
            if(j==0){
                dp[i][j]=inverse(fact[cnt[i-1]],mod)*dp[i-1][j];
            }
            else if(i>=j){
                dp[i][j]=inverse(fact[cnt[i-1]-1],mod)*dp[i-1][j-1]+inverse(fact[cnt[i-1]],mod)*dp[i-1][j];
            }
        }
    }
    ...
    for(auto it:mp){
        ans=ans*inverse(fact[it.ss],mod);
        ans%=mod;
    }
    ans=ans*dp[cnt.size()][n];
    ans%=mod;
    cout<<ans<<endl;
