...
ll r=1e9;
double calc(ll rac,double c,double m,double p,double v){
    // if((c==0 and m==0) or p==1){
    //     return p*r;
    // }
    double ans=rac*(p/r);
    if(c!=0){
        if(c>=v){
            if(m!=0)
           ans+= (c/r)*calc(rac+1,c-v,m+v/2,p+v/2,v);
           else
           ans+=(c/r)*calc(rac+1,c-v,m,p+v,v);
        }else{
            if(m!=0)
            ans+=(c/r)*calc(rac+1,0,m+c/2,p+c/2,v);
            else 
            ans+=(c/r)*calc(rac+1,0,m,p+c,v);
        }
    }
    if(m!=0){
        if(m>=v){
            if(c!=0)
            ans+=(m/r)*calc(rac+1,c+v/2,m-v,p+v/2,v);
            else
            ans+=(m/r)*calc(rac+1,c,m-v,p+v,v);    
        }else{
            if(c!=0)
            ans+=(m/r)*calc(rac+1,c+m/2,0,p+m/2,v);
            else
            ans+=(m/r)*calc(rac+1,c,0,p+m,v);
        }
    }
    return ans;
...
void solve(){
    double c,m,p,v;inp4(c,m,p,v);
    c*=r;
    m*=r;
    p*=r;
    v*=r;
    // err4(c ,m,p,v);
    cout<<setprecision(12)<<calc(1,c,m,p,v)<<endl;
