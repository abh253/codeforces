...
ll pairs(vector<pll>&v,set<ll>st){
    ll n=v.size();
    ...
    while(i<n and j<n){
        ...
        while(j<n){
            auto x=st.upper_bound(v[i].ss);
            if(x==st.end() or *x>v[j].ss){
                mp[v[j].ff]++;
                j++;
            }else{
                break;
...
void solve(){
    ...
    sort(all(verp),logic);
    sort(all(horp),logic);
    ll ans=pairs(verp,hor)+pairs(horp,ver);
...
// obs
// if one point is on vetical street and one is on horizontal they never form a inconvient pair
// so point on parallel streets can form a inconvient pair
// if two points are on different parallel streets then if there is no street of opposite direction between those two points then 
// they form an inconvient pair
// now how to calculate
// if a point lie in both vertical and horizontal street's intersection then it won't participate in any inconvient pair
// now if i have all points which are in the vertical streets 
// then sort them on the value of there ys
// then use two pointers 
// start with i=0,j=0
// then, increase j until there is no street of opposite direction between them if we found one
// then do nc2 of (j-i) and set i to j and continue

// we need to subtract the extra answer which is the pairs which lie on the same line.