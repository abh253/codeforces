...
pll width(multiset<pll>&hw,multiset<pll>&wh);
pll height(multiset<pll>&hw,multiset<pll>&wh){
    ll max_h=(*hw.rbegin()).ff;
    ll current_h=max_h;
    ll current_w=0;
    while(!hw.empty() and (*hw.rbegin()).ff==max_h){
        pll x=*hw.rbegin();
        current_w+=x.ss;
        hw.erase(hw.find(x));
        wh.erase(wh.find({x.ss,x.ff}));
    }

    if(hw.size()){
        pll y=width(hw,wh);
        if(y.ff==-1 or y.ff!=current_h){
            return {-1,-1};
        }
        current_w+=y.ss;
    }
...
pll width(multiset<pll>&hw,multiset<pll>&wh){
    ll max_w=(*wh.rbegin()).ff;
    ll current_h=0;
    ll current_w=max_w;
    while(!wh.empty() and (*wh.rbegin()).ff==max_w){
        pll x=*wh.rbegin();
        current_h+=x.ss;
        wh.erase(wh.find(x));
        hw.erase(hw.find({x.ss,x.ff}));
    }

    if(hw.size()){
        pll y=height(hw,wh);
        if(y.ff==-1 or y.ss!=current_w){
            return {-1,-1};
        }
        current_h+=y.ff;
    }
...
void solve(){
    ...
    FOR(i,n){
        cin>>v[i][0]>>v[i][1];
        hw.insert({v[i][0],v[i][1]});
        wh.insert({v[i][1],v[i][0]});
    }
    multiset<pll>hwcopy=hw;
    multiset<pll>whcopy=wh;
    pll ans1=height(hwcopy,whcopy);
    pll ans2=width(hw,wh);
    set<pll>ans;
    if(ans1.ff!=-1){
        ans.insert(ans1);
    }
