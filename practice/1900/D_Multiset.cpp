...
ll build(int v,int tl,int tr,vint &t,vint &a){
    if(tl==tr){
      return  t[v]=a[tl];   
    }
    ll tm=(tl+tr)/2;
...
ll sum(int v,int l,int r,int tl,int tr,vint &t){
    if(l>r)return 0;
    if(l==tl and r==tr){
        return t[v];
    }
...
void update(int v,int pos,int new_val,int tl,int tr,vint &t,vint& count){
    if(tl==tr){
        count[tl]+=new_val;
        t[v]+=new_val;
        return;
    ...
    ll mid=(tl+tr)/2;
    if(pos<=mid){
        update(2*v+1,pos,new_val,tl,mid,t,count);
    }else{
        update(2*v+2,pos,new_val,mid+1,tr,t,count);
    }
    t[v]=t[2*v+1]+t[2*v+2];
}
int kth(int v,int tl,int tr,vint &t,int k,int forward){
    if(t[v]+forward<k)return -1;
    ...
    int idx=-1;
    if(t[f]+forward>=k){
        idx= kth(f,tl,mid,t,k,forward);
    }else{
        idx= kth(s,mid+1,tr,t,k,t[f]+forward);
    }
...
//    vll v(n);vinput(v);
   ...
   FOR(i,q){
        ll x;cin>>x;
        if(x>0){
            update(0,x,1,0,n,t,count);
        }else{
            x=abs(x);
            ll l=kth(0,0,n,t,x,0);
            update(0,l,-1,0,n,t,count);
        }
   }
//    errA(t);
    For(i,1,n){
...
//so i need only one element left in the array at the end;
//two pointers
