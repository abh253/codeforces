...
ll sam=0;
ll duri=0;
void dfs(int i,int p,vector<vll>&t){
    tin[i]=sam++;
    dist[i]=duri;
    duri++;
    for(auto j: t[i]){
...
void solve(){
    ...
    FOR(i,m){
        ll mdn=0;//maximum depth node;
        FOR(j,query[i].size()){
            query[i][j]=parent[query[i][j]];
            if(dist[query[i][j]]>dist[mdn])mdn=query[i][j];
        }
        bool flag=1;
        // cout<<mdn<<endl;
        for(auto j:query[i]){
            if(!(tin[j]<=tin[mdn] and tout[j]>=tout[mdn]))flag=0;
            // cout<<j<<" ";
        }
        // cout<<endl;
        if(flag)yess;
