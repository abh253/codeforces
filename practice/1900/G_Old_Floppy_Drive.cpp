...
void solve(){
    ll n,m;cin>>n>>m;
    vll a(n);vinput(a);
    vll query(m);vinput(query);
    // ll suma=accumulate(all(a),0LL);
    
    map<ll,ll>mp;

    // how many loops can be ignored
    // map<ll,ll>nami;
    ...
    FOR(i,n){
        ...
        // if(mp.count(sum))mp[sum]=min(i,mp[sum]);
        // else mp[sum]=i;

    }
    vll ans(m);
    // ll maxpf=mp.rbegin()->first;
    ll maxpf=mxsum;

    FOR(i,m){
        
        auto x=mp.lower_bound(query[i]);

        if(x==mp.end()){
            if(sum<=0){
                ans[i]=-1;
            }else{
                ll round=(query[i]-maxpf-1+sum)/sum;
                query[i]-=round*sum;
                auto y=mp.lower_bound(query[i]);
                ans[i]=round*n+y->second;
            }
        }else{
           ans[i]=x->second; 
        }
...
// for every index let say 
// on first traversal at every point i can make it sum[i] and after one complete iteration 
// when i encounter that postion again the sum will be sum[i]+sumarray,sum[i]+2*sumarray ...