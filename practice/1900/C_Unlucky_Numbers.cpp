...
void help(ll i,ll l,ll r,ll max_n,ll min_n,string&left,string&right,string &current_str){
    ...
    if(dp[i][l][r][max_n][min_n]==1){
        // cout<<i<<" "<<l<<" "<<r<<" "<<max_n<<" "<<min_n<<endl;
        // cout<<dp[i][l][r][max_n][min_n]<<endl;
        return;
    }
    ...
    for(ll j=x;j<=y;j++){
        ll new_l=(l==1)&& (j==x)?1:0;
        ll new_r=(r==1)&& (j==y)?1:0;
        current_str.push_back('0'+j);
        help(i+1,new_l,new_r,max(max_n,j),min(min_n,j),left,right,current_str);
        current_str.pop_back();
    }
    dp[i][l][r][max_n][min_n]=1;
}
...
void solve(){
    ...
    help(0,1,1,0,9,leftStr,rightStr,current_str);
