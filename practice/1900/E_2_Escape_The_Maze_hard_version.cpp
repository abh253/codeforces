...
void solve(){
    ...
    for(auto x:frd)fvis[x-1]=x-1,q2.push(x-1);
    // errA(fvis)
    // return;
    vlad[0]=1;
    bool ispos=0;
...
// vlad need to go to the leaf if a subset of given friends can caught him before getting to the leaf
// thats vlad's loss
// and 
// it's a tree so the friend will move upward to caught vlad 
// because that is the path to go to leaf
// if its possible for them to stop vlad every subtree connected to 1 need a friend
// so if its possible answer will be t[0].size() otherwise -1