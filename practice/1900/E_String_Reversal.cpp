...
void solve(){
    ...
    FOR(i,n){
        ...
        ll real_idx=idx+(st.size()-st.order_of_key(idx));
        ans+=real_idx-i;
        st.insert(idx);
    }
    cout<<ans<<endl;
...
// i know how my end string would look like

// algorithm
// first make a string which will be reverse of the current string 
// then kepp the index of every letter in a 2d matrix vector<vll>v(26) and push index of evvery 
//letterr ,start from back of the old string
// then iterate 0 to n-1 index and see the index of required alphabet in my matrix
// its real index will be = idx_in_matrix+indexes greater than it which is already moved to left
// can be done by orderedset
//