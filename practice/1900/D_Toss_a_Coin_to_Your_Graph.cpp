...
//1. if cycle present in the vertex which have value less that mid;
//2. if no cycle there is a way to reach k vertex from any vertex via dfs;
...
bool cycle(int i,int mid,vll &visited,vll &stk){
    visited[i]=1;
    stk[i]=1;
    for(auto j:t[i]){
        if(v[j]>mid)continue;
        if(visited[j] and stk[j])return 1;

        if(!visited[j] and cycle(j,mid,visited,stk)){
            return 1;
        }
    }
    stk[i]=0;
...
// if dfs is called that means there is no cycle
ll dfs(int i,int mid,vll &dp){
    
    ll mx=0;
    for(auto j:t[i]){
        if(v[j]>mid)continue;

        if(dp[j])mx=max(dp[j],mx);
        else
        mx=max(mx,dfs(j,mid,dp));
    }
    return dp[i]=mx+1;
}
...
bool check(int mid){
    ...
    vll stk(n);
    for(int i=0;i<n;i++){
        if(v[i]<=mid and (!visited[i])){
            bool iscycle=cycle(i,mid,visited,stk);
            if(iscycle)return 1;
        }
    }
    // errA(visited);
...
// binary search
// make a graph of v[i]<=mid;
// then check for loops 
// or maximum vertex can be seen by unvisited nodes
// and make decision based on them
// in dfs i won't go to those vertex which have higher value
// and my future self other complications diy .
