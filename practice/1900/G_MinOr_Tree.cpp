...
void solve(){
   ...
   ll n,m;cin>>n>>m;
    ...
    //binary search
    // for checking a mid if it is valid or not
    // we will take all the weights in which set bit is only set on the places where it is set on mid
    // after that if graph is connected then mid can be taken as valid
    // invalid otherwise then we will check for greater number
    ...
    for(int i=30;i>=0;i--){
        if(((1<<i)&orr)!=0){
            vll Pa(n);FOR(i,n)Pa[i]=i;
            vll S(n,1);
            for(int j=0;j<m;j++){
                if((edges[j][2]&(1<<i))==0 and (notused&edges[j][2])==0){
                    ll x=edges[j][0],y=edges[j][1];
                    Union(x,y,S,Pa);
                }
            }

            set<int>p;
            for(int i=0;i<n;i++)p.insert(findParent(i,Pa));
            // cout<<p.size()<<endl;
            if(p.size()>1){
                ans|=(1<<i);
            }else{
