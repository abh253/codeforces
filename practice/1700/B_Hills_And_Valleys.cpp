...
void solve(){
    ...
    vll sf(n);
    for(int i=1;i<n-1;i++){
        if(v[i]>v[i-1] and v[i]>v[i+1])prif[i]++;
        if(v[i]<v[i-1] and v[i]<v[i+1])prif[i]++;
        prif[i]+=prif[i-1];
    }
    prif[n-1]+=prif[n-2];
    for(int i=n-2;i>=1;i--){
        if(v[i]>v[i-1] and v[i]>v[i+1])sf[i]++;
        if(v[i]<v[i-1] and v[i]<v[i+1])sf[i]++;
        sf[i]+=sf[i+1];
    }
    sf[0]+=sf[1];
    ...
    for(int i=1;i<n-1;i++){
        ll x=i-2>=0?prif[i-2]:0;
        ll y=i+2<n?sf[i+2]:0;
        if(i+2<n){
             if((v[i-1]<v[i+1] and v[i+2]<v[i+1])or (v[i-1]>v[i+1] and v[i+2]>v[i+1])){
                ans=min(ans,x+y+1);
             }else{
                ans=min(ans,x+y);
             }
        ...
        if(i-2>=0){
            if((v[i-1]<v[i+1] and v[i-1]<v[i-2]) or (v[i-1]>v[i+1] and v[i-1]>v[i-2])){
                ans=min(ans,x+y+1);
            }else{
                ans=min(ans,x+y);
            }
