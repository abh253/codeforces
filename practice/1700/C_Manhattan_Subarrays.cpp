...
bool ckinc(ll &a,ll &b,ll&c){
    return a<=b and b<=c;
}
bool ckdec(ll &a,ll &b,ll&c){
    return a>=b and b>=c;
}
int main(){
	...
	while(testcase-->0){
        ...
        For(i,1,n-2){
            if(!((v[i-1]<=v[i] and v[i]<=v[i+1])or (v[i-1]>=v[i] and v[i]>=v[i+1]))){
                ans++;
                if(i+2<n){
                    if(!(ckinc(v[i-1],v[i],v[i+2]) 
                    or ckinc(v[i-1],v[i+1],v[i+2])
                    or ckinc(v[i],v[i+1],v[i+2]) 
                    or ckdec(v[i-1],v[i],v[i+2])
                    or ckdec(v[i-1],v[i+1],v[i+2])
                    or ckdec(v[i],v[i+1],v[i+2])))ans++;
                }
            }
        }
