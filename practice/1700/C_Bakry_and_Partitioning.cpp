...
vll v;
ll wtz;
int helper(ll i,vector<vector<int>>&zoro,ll &level,ll &nd,ll ntskip=-1,ll clevel=0,ll parent=-1){
    if(i==ntskip)return 0;

    ll xo=v[i];
    for(auto it:zoro[i]){
        if(it==parent)continue;
        xo^=helper(it,zoro,level,nd,ntskip,clevel+1,i);
    }
    
    if(xo==wtz and clevel>level){
        nd=i;
        level=clevel;
    }
    return xo;
...
void solve(){
    ...
    v=vll(n);
    ll xo=0;
    FOR(i,n){cin>>v[i];xo^=v[i];}

    vector<vector<int>>zoro(n);
