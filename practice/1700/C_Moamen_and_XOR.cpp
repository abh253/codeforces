...
void solve(){
    ...
    vector<vll> dp(k+1,vll(2));//0=in next bit the ans>xor and 1=next it ans==xor

    dp[0][0]=1;
    dp[0][1]=1;
    ll tpn1=power(2,n-1,mod);
    // ll tpn=power(2,n,mod);
    for(int i=1;i<=k;i++){
        if(n%2==0){
            dp[i][1]=dp[i-1][1]*(tpn1-1)+dp[i-1][0];
        }
        else{ 
            dp[i][1]=dp[i-1][1]*(1+tpn1);
        }
        dp[i][0]=power(2,n,mod)*dp[i-1][0];
        dp[i][0]%=mod;
        dp[i][1]%=mod;
    }
    cout<<dp[k][1]<<endl;
