...
void solve(){
    ...
    vll zoro(61);
    rep(i,60,0){
        ll sbit=((1LL<<i)&v)==0?0:1;
        ll xbit=((1LL<<i)&u)==0?0:1;

        if(sbit==0 and xbit==0){
	        if (carry ==0){
                carry=0;
            }
	        else if(carry==1){
...
// 2nd bit-1 on
// 0th bit- need 1 on bit but sum bit is zero means it have to be even number of times therefore one bit
// is definetly a carry but if carry is there it will make next bit one more

// // i can make cases 
// moving from left bit to right bit and keeping a carry variable
// if(both off}{
// 	if carry ==0
// 		carry=0
// 		then no bit is on
...
// 		2 bits on 
// }
// if(sum bit off and xor bit on){
// 	if(carry==1)
// 		carry=1
// 		1 bit on at this place
// 	if(carry==0)
// 		notpossible
			
// }

// if (both bit are open)
// 	if(carry==0)
// 		one bit on
// 	if(carry==1)
// 		3 bit on