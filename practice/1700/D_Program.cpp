...
void solve(){
    ...
    string s;
    cin>>s;
    vector<vector<int>>suf(n+2,vector<int>(2));
    ...
    for(int i=1;i<=s.size();i++){
        if(s[i-1]=='+'){
            x++;
            pref[i][0]=pref[i-1][0];
            pref[i][1]=max(pref[i-1][1],x);
        }
        else {
            x--;
            pref[i][1]=pref[i-1][1];
            pref[i][0]=min(pref[i-1][0],x);
        }
        cc[i]=x;
    }
        ...
        for(int i=n;i>=1;i--){
            if(s[i-1]=='+'){
                suf[i][0]=min(1+suf[i+1][0],0);
                suf[i][1]=max(suf[i+1][1]+1,1);
            }else{
                suf[i][0]=min(-1+suf[i+1][0],-1);
                suf[i][1]=max(-1+suf[i+1][1],0);
            }
        }

        FOR(j,m){
            ll l,r;cin>>l>>r;
            ll maxr=cc[l-1]+suf[r+1][1];
            ll minr=cc[l-1]+suf[r+1][0];
            maxr=max(maxr,(ll)pref[l-1][1]);
            minr=min(minr,(ll)pref[l-1][0]);
