...
inline ll to1(int i,int j, int x,int y){
    return (x-i+1)*(y-j+1)-(prefix[x][y]+prefix[i-1][j-1]-prefix[i-1][y]-prefix[x][j-1]);
}
inline ll to0(int i,int j, int x,int y){
    return (prefix[x][y]+prefix[i-1][j-1]-prefix[i-1][y]-prefix[x][j-1]);
}
...
void solve(){
    ...
    vector<string>v(n);vinput(v);

    prefix=vector<vector<int>>(n+1,vint(m+1));

    For(i,1,n){
        For(j,1,m){
            prefix[i][j]=(v[i-1][j-1]-'0')+prefix[i-1][j]+prefix[i][j-1]-prefix[i-1][j-1];
        }
    }
    ...
    For(i,1,m){
        For(j,i+3,m){
            ll prvcost=to0(2,i+1,4,j-1)+to1(5,i+1,5,j-1)+to1(1,i+1,1,j-1)+to1(2,j,4,j)+to1(2,i,4,i);
            ans=min(ans,prvcost);
            prvcost-=to1(5,i+1,5,j-1);
            prvcost+=to1(5,i,5,i)+to1(5,j,5,j)+to0(5,i+1,5,j-1);
            For(k,6,n){
                ll curcost=to0(k-3,i+1,k-1,j-1)+to1(k,i+1,k,j-1)+to1(k-4,i+1,k-4,j-1)+to1(k-3,j,k-1,j)+to1(k-3,i,k-1,i);

                ll lastrow_cost=to1(k,i+1,k,j-1);

                curcost=min(curcost,prvcost+lastrow_cost);

                prvcost=curcost;

                ans=min(prvcost,ans);

                prvcost-=to1(k,i+1,k,j-1);
                prvcost+=to1(k,i,k,i)+to1(k,j,k,j)+to0(k,i+1,k,j-1);
