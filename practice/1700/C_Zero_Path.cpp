...
void solve(){
    ...
    ll n,m;cin>>n>>m;
    // vll v(n);vinput(v);
    vector<vector<int>>v(n,vint(m));
    FOR(i,n){FOR(j,m){cin>>v[i][j];}}

    vector<vector<vint>>dp(n,vector<vint>(m,vint(2)));
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            if(i==0 and j==0){
                ...
                dp[i][j][0]=dp[i][j-1][0]+v[i][j];
                dp[i][j][1]=dp[i][j-1][1]+v[i][j];
            }else if(j==0){
                dp[i][j][0]=dp[i-1][j][0]+v[i][j];
                dp[i][j][1]=dp[i-1][j][1]+v[i][j];
            }else{
                dp[i][j][0]=v[i][j]+min(dp[i][j-1][0],dp[i-1][j][0]);
                dp[i][j][1]=v[i][j]+max(dp[i][j-1][1],dp[i-1][j][1]);
            }
        }
    }
    if(dp[n-1][m-1][1]%2 or dp[n-1][m-1][1]<0 or dp[n-1][m-1][0]>0){
        noo;
    }else yess;
