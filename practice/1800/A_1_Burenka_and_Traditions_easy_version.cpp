...
void solve(){
    ...
    v.pb(0);
    ll N=9000;
    vector<vint>dp(n+1,vint(N,INT_MAX));
    dp[0][v[0]]=0;
    ...
    For(i,0,n-1){
        FOR(j,N){
            if(dp[i][j]!=INT_MAX){
                if(j==0){
                    dp[i+1][v[i+1]]=min(dp[i][j],dp[i+1][v[i+1]]);
                }
                else{
                    dp[i+1][v[i+1]]=min(dp[i][j]+1,dp[i+1][v[i+1]]);
                    dp[i+1][v[i+1]^j]=min(dp[i][j]+1,dp[i+1][v[i+1]^j]);
                }
            }
        }
    }
...
//observations
// taking sets of 2 is benificial
// taking sets more than 2 size
// taking a set of 2 many times is same as taking a bigger set
// and to make a number ^x == 0 it is obvious that this number need to be xor with number
// does not matter if that number ==xor of many other x
// 