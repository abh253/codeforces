...
int dfs(int i,int p,vector<vll>&t,map<pair<int,int>,int>&dp){
    if(dp.count({i,p})){
        return dp[{i,p}];
    }
    ll ans=1;
...
void solve(){
    ...
    while(pf.size()<usage.size())pf.pb(1);

    sort(all(pf));

    while(pf.size()>usage.size()){
...
//     }
// }
// so with give pf i can combine two pf but can't split em
//my approach
// so i basically need to give edge maximum number when which is used the most 
// standing at an edge if i know nodes in left part and number of nodes in right part
// i can define usage =left*right
// and after creating usage array of all edges i can take maximum number from
// the given pf and multiply it with usage to get the max di
