...
void solve(){
    ...
    FOR(i,k){
        ...
        free[v[i].ff]=free[v[i].ss]=0;
    }

    ll totalfree=0;
    for(int i=1;i<=2*n;i++){
        if(free[i]==1)totalfree++;
        free[i]+=free[i-1];
    }
    ll ans=(n-k)*(n-k-1)/2;
    for(int i=0;i<k;i++){
        for(int j=i+1;j<k;j++){
            if((v[i].ff>v[j].ff and v[i].ff<v[j].ss) ^(v[i].ss>v[j].ff and v[i].ss<v[j].ss))ans++;
        }
        // cout<<v[i].ff<<" "<<v[i].ss<<endl;
        ll x=free[v[i].ss]-free[v[i].ff];
        ans+=min(totalfree-x,x);
    }
    // errA(free);
    cout<<ans<<endl;
