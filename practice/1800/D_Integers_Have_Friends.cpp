...
ll build(int v,int l,int r,vll &t,vll & arr){
    ...
    ll mid=(l+r)/2;
    ll a=build(2*v,l,mid,t,arr);
    ll b=build(2*v+1,mid+1,r,t,arr);
    t[v]=__gcd(a,b);
}

ll range_gcd(int v,int tl,int tr,int l,int r,vll &t){
    if(l>r){
        return 0;
    }
    if(tl==l and tr==r){
    ...
    ll a=range_gcd(2*v,tl,mid,l,min(mid,r),t);
    ll b=range_gcd(2*v+1,mid+1,tr,max(mid+1,l),r,t);
...
void solve(){
    ...
    build(1,0,d.size()-1,t,d);
    // errA(t);
    for(int i=0;i<d.size();i++){
        ll l=i,r=d.size()-1;
        while(l<r){
            ll mid=(l+r+1)/2;
            ll x=range_gcd(1,0,d.size()-1,i,mid,t);
