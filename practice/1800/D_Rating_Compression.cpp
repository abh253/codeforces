...
void solve(){
    ...
    for(int i=1;i<=n;i++){
        ...
        while(!sl.empty() and v[sl.top()]>=v[i])sl.pop();

        ll j=n-i+1;
        while(!sr.empty() and v[sr.top()]>=v[j])sr.pop();
    ...
    for(int i=1;i<=n;i++){
        ll d=right[i]-left[i]-1;
        diff[v[i]]=max(diff[v[i]],d);
    }
    for(int i=2;i<=n;i++){
...
//     }
// }
// every number shoudl appear once
// permutation is possilbe for length for which we have continuous elements
//1 to len
// smaller left and smaller right
//if their difference is less than k then that number would appear 
//in the array to be formed
//