...
vll color;
// ll dfs1(int i,int p,vector<vll>t,map<pll,ll>&white){//white
//     ll ans=color[i];
//     for(auto j:t[i]){
//         if(j!=p){
//             ans+=dfs1(j,i,t,white);
//         }
//     }
//     return white[{i,p}]=ans;
// }
// ll dfs2(int i,int p,vector<vll>t,map<pll,ll>&black){//black
//     ll ans=!color[i];
//     for(auto j:t[i]){
//         if(j!=p){
//             ans+=dfs2(j,i,t,black);
//         }
//     }
//     return black[{i,p}]=ans;
// }

//real solution
ll dfs1(int i,int p,vector<vll>&t,vll &ans){
    ans[i]=color[i]==1?1:-1;

    for(auto j:t[i]){
        if(j!=p){
            ll sa=dfs1(j,i,t,ans);
            if(sa>0)ans[i]+=sa;
        }
    }
...
void dfs2(int i,int p,vector<vll>&t,vll &ans){

    for(auto j:t[i]){
        if(j!=p){
            if(ans[j]<0){
                ans[j]=max(ans[j],ans[j]+ans[i]);
            }
            else{
                if(ans[i]-ans[j]>0){
                    ans[j]+=ans[i]-ans[j];
...
void solve(){
    ...
    dfs2(0,-1,t,ans);
        ...
        //i parent
    // map<pair<ll,ll>,ll>white;
    // map<pair<ll,ll>,ll>black;
    // vll ans(n,INT_MIN);
    // for(int i=0;i<edges.size();i++){
    //     ll a=edges[i][0];
    //     ll b=edges[i][1];

    //     ll whitea=dfs1(a,b,t,white);
    //     ll blacka=dfs2(a,b,t,black);
    //     ll whiteb=dfs1(b,a,t,white);
    //     ll blackb=dfs2(b,a,t,black);

    //     ans[a]=max(whitea-blacka,ans[a]);
    //     ans[b]=max(whiteb-blackb,ans[b]);
...
// then it will call for every child
// and will only add to the answer which give positive 
// ignore the negative
//