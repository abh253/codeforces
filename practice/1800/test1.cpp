...
int dp[5][101];
int dfs(int last,int i,string s,map<char,int>&order){

    if(i==s.length()){
        return last==4?0:INT_MIN;
    }
    if(order[s[i]]<last)return dfs(last,i+1,s,order);

    if(dp[last][i]!= -1){
        return dp[last][i];
    }

    return dp[last][i]=max(dfs(order[s[i]],i+1,s,order)+1,dfs(last,i+1,s,order));
