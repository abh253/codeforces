...
void solve(){
    ...
    while(1){
        ...
        for(int i=1;i<zoro.size();i++){
            zoro[i]+=zoro[i-1];
        }

        for(int i=1,j=0;i<=v.size();i++){
            if(i==v.size() or v[i]>v[i-1]){
                zoronext.pb(zoro[i-1]-(j-1>=0?zoro[j-1]:0));
                vnext.pb(v[j]);
                j=i;
            }
        }
        ...
        // errA(zoro);
        // errA(v);
        if(zoro.size()==zoronext.size()){
            break;
        }
    }
    
    // errA(zoro);
    // now i have to check if i can take a subsequence from zoro having combined sum ==n

    // if(sum==n){
    //     return 1;
    // }
    // if(i==zoro.size())return 0;
    // //j=cursum
    // return fun(i+1,j) or fun(i+1,j+zoro[i]);
    

    //dp[i][j] be the if j sum can be made till index i-1 or not
    ll pn=n;
    n=zoro.size();
    // errA(zoro);
    vector<vll>dp(n+1,vll(pn*2+1));
...
//as i observed a decreasing subarray of merged will be part of same array
// so i will put them together after that starting number of every part will be observed
// if they are in decreasing order then they will be part of same array
