...
void solve(){
    ...
    For(i,1,n){
        ...
        if(t[i]>=r){
            ...
            ll now_l=from+d*(t[i]-l);
            ll now_r=from+d*min(t[i+1]-l,r-l);

            if(now_l>now_r)swap(now_l,now_r);

            if(now_l<=x[i] and now_r>=x[i]){
                ans++;
            }
...
// algorithm:
// we start with first instruction and keep a variable at how much time the robot reach to the required position
// we can keep variables from,to, direction(0,-1,1) and time to reach 
// if t[i]>= time reach then the robot can accept the command;
// then calulate time to reach to the required point if it is less than  or euqal t[i+1]] then ans++

// else if can not take the command
// in one case current command's position to be reached:
// if lie in the path then we can calculate of reaching to it will take if it is less thann t[i+1] then ans++
// else move on 