...
vll p;
vll h;
ll dfs1(int i,int parent,vector<vll>&tr){
    ll ans=p[i];
    for(auto child:tr[i]){
        if(child==parent)continue;
        ans+=dfs1(child,i,tr);
    }
    return p_subtree[i]=ans;
}
//yes/no,good,bad
vector<int> dfs2(int i,int parent,vector<vll>&tr){

    int tpeople=p_subtree[i];
    if((h[i]+tpeople)%2)return {0,0,0};
    int good=(h[i]+tpeople)/2;
    
    int bad=tpeople-good;
    if(good<0 or bad<0)return {0,0,0};

    int childgood=0;
    int childbad=0;
    for(auto child:tr[i]){
        if(child==parent)continue;
        vector<int>sa=dfs2(child,i,tr);
        if(sa[0]==0){
            return sa;
        }
        childgood+=sa[1];
    ...
    if((childgood+childbad!=good+bad-p[i]) or 
        childgood>good
    )return {0,0,0};
...
void solve(){
    ...
    p_subtree=vll(n);
    dfs1(0,-1,tr);
    vint ans=dfs2(0,-1,tr);

    if(ans[0] and ans[1]+ans[2]==m){
        yess;
    }else noo;
