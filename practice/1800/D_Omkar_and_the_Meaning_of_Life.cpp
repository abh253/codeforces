...
void solve(){
    ...
    q=vint(n,1);
    for(int k=2,l=ans[n-1]+1;l<=n;l++,k++){
...
// i want to find some relation which can help me do  my thing
// so i had to  see editorial
// i was thinking of using same number for the query and have 
// unique at the index for which i need specific info
// so i am getting leftmost index
// so making query related to last element makes sense
//idea-
// i will take x at every index and 1 at nth place
// if there exist repeated number it will be same as Sk
// Pi+k=Pn+1
// Pi=Pn-k+1
// if we check for k=2 till we get at the point where no same sum occurs 
//  pi=Pn-k+1
//  pi range from pn-1 to 1 after that there will be no same sum
// and pn= upper range of k
// and for finding the element greater than pn 
// put 1 every where and take k for nth position
// pi+1=pn+k
// pi=pn+k-1