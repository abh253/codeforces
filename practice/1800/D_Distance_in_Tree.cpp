...
ll k;
void dfs1(int i,int p,vector<vll>&t,vector<vll>&dist){
    dist[i][0]=1;
    for(auto j:t[i]){
        if(j!=p){
            dfs1(j,i,t,dist);
...
void dfs2(int i,int p,vector<vll>&t,vector<vll>&dist){
    ...
    for(auto j:t[i]){
        if(j!=p){
            ...
            for(int x=1;x<=k;x++){
                // dist[j][x]+=dist[i][x-1]-(x-2>=0?dist[j][x-2]:0);
                ll z=dist[i][x-1]-lt.ff;
                lt.ff=lt.ss;
                lt.ss=dist[j][x];
                dist[j][x]+=z;
            }
