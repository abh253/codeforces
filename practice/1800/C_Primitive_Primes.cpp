...
// }
//obs
// coefficients don't have common factors
// was not able to solve
// after seeing editorial
// in the a array and b array 
// they have gcd=1
//that means there are two or more set of numbers which do not have common factors
//lets take a contradictory case if all of a or b are divisible by p then every number in final array will also have all coefficient divisible by p
// this step is hard to think of ( for me atleast )
//both array should have atleast 1 number not divisible by p
// let say ai and bj are not divisible by p(ai and bj are first number not divisible by p)
// their x will have power i+j
// ai+j*b0 +ai+j-1*b1+...ai+1*bj-1 +ai*bj+ ai-1*bj+1...a0*bi+j
//all the number except ai*bj are divisible by p so the number as whole won't be divisible by p
