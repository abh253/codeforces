...
void solve(){
    ...
    for(int i=0;i<n-1;i++){
        ...
        ll index=upper_bound(x.begin(),x.end(),next)-x.begin();
        
        index--;
    
        ans=max(ans,index-i+1+(index+1<n?v[index+1]:0));
    }
    cout<<ans<<endl;
...
// i only have to see x cordinate because i know y>=1
// sort x in  increasing order
// then put the first plank on first x and try to find where to put the second
// i need to do the second part in logn or precompute it
// precomputing it is
// andso on
//