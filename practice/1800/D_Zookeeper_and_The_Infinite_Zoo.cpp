...
void solve(){
    ...
    ll ns=s;
    for(int i=0;i<30;i++){
        int nsbit=0;if(ns&(1<<i))nsbit=1;
        int ebit=0;if(e&(1<<i))ebit=1;
        int sbit=0;if(s&(1<<i))sbit=1;
...
//what i did
//i can reach from s to e if i can convert s to e
// moving 0th bit to 29th bit 
//because where the bits of s  and e are equal no need to change
//otherwise need to change
//there can be two cases bit of s is 1 and e is 0 
// i this case we will just make this bit 0 and next bit +1
//other case s is 0 and e is one 
//if the initial s has 1 at this position only then we can do the operation
//otherwise not possible

//editorial or 2nd approach
//only possible when e >s
//if the number of bits in s>=number of bits in e
//i will do this approach in another file