...
void solve(){
    ...
    For(i,1,n){
        if(i<n){
            v[i-1]=
            (180*power(10,n-i-1,modu)+
            810*(n-i-1)*(n-i-2>=0?power(10,n-i-2,modu):0))%modu;
        }else{
            v[i-1]=10;
        }
    }
...
// 0 is dividing blocks when it is used for padding
//i was not able to get that the block only belongs to one integer
//there are 10 block of n size
// i need to generalize of 1 to n
// generalize the formula 10*(2*9+(n-i-1)*81) for i 3 to n;