...
int helper(int i,int T,vll &v){
    ...
    if(T>=v[i]){
        return dp[i][T]=T-v[i]+helper(i+1,T+1,v);
    }else{
        ll ans=INT_MAX;
        For(j,T,v[i]){
            ans=min(ans,abs(v[i]-j)+helper(i+1,j+1,v));
        }
        return dp[i][T]=ans;
    }
...
void solve(){
    ...
    sort(all(v));
    dp=vector<vll>(n+1,vll(2*n+1,-1));

    cout<<helper(0,1,v)<<endl;
