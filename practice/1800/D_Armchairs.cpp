...
vector<vll>dp;
ll sol(int i,int j,vll &a,vll &b){
    ...
    return dp[i][j]=min(sol(i,j+1,a,b),abs(b[j]-a[i])+sol(i+1,j+1,a,b));
...
void solve(){
    ...
    for(int i=l-1;i>=0;i--){
        for(int j=m-1;j>=0;j--){
            dp[i][j]=min(dp[i][j+1],abs(b[j]-a[i])+dp[i+1][j+1]);
        }
    }
