...
void solve(){
    ...
    sort(all(onec));onec.pb(0);
    reverse(all(onec));

    sort(all(twoc));twoc.pb(0);
    reverse(all(twoc));
    // errA(twoc);
    For(i,1,onec.size()-1){
        onec[i]+=onec[i-1];
    ...
    ll totalmemory=accumulate(all(a),0LL);
    if(totalmemory<m){
        mone;
        return ;
    }
    ll tcon=accumulate(all(b),0LL);
    ll l=0,r=tcon;
    ...
    while(l<r){
        ...
        for(ll i=0;i<=mid;i++){
            ...
            if(((mid-i)<onec.size() and (i/2)<twoc.size()) and onec[mid-i]+twoc[i/2]>=m){
                flag=1;
                break;
            }
        }
