...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        ll range1=s.size()-s.order_of_key(vx[i]);

        ll range2=upper_bound(vx.begin()+i+1,vx.end(),vy[i])-(vx.begin()+i+1);

        ll total=range1+range2+1;
...
//if i sort all the numbers according to first number
// for a certain index 
//on the left
// the number of second number which is greater than or equal to v[index].ff that are the one which are being touched by index
// on the right
// after that the number whose first number is less than second