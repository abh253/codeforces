...
int dfs(int r,int c,vector<string>&v){
    ...
    int z=v[r][c]=='*';
    if(r==v.size()-1){
        return z + dfs(r,c+1,v);
    }
    else if(c==v[0].size()-1){
        return z +dfs(r+1,c,v);
    }
    int row=INT_MAX;
    int col=INT_MAX;
    for(int i=r+1;i<v.size();i++){
        if(v[i][c]=='*')row=min(row,i-r);
    }
    for(int i=c+1;i<v[r].size();i++){
        if(v[r][i]=='*')col=min(col,i-c);
    }
    if(row<col){
        return z +dfs(r+1,c,v);
    }else return z+ dfs(r,c+1,v);
}
void solve(){
