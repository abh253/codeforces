...
// }
vector<vector<pll>>v;
vector<map<int,unordered_map<int,int>>>cnt;
vll ans;
string s;
void dfs(int i,vector<vll>&t,int h=1){

    int bst=-1;
    for(auto j:t[i]){
        dfs(j,t,h+1);
        if(bst== -1 or (cnt[j].size() > cnt[bst].size())){
            bst=j;
        }
    }

    if(bst!=-1)swap(cnt[i],cnt[bst]);

    for(auto j:t[i]){
        if(j!=bst){
            for(auto it:cnt[j]){
                // it.ff height
                // it.ss is map where key is 0 to 25 and value is cnt of alphabets
                int ch=it.ff;
                for(auto it2:it.ss){
                    int x=it2.ff;
                    int y=it2.ss;
...
void solve(){
    ...
    ll n,q;cin>>n>>q;
    cnt=vector<map<int,unordered_map<int,int>>>(n);
    ans=vll(q);
    vector<vll>t(n);
    For(i,1,n-1){
        ll x;cin>>x;x--;
