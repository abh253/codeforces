...
void dpcode(vector<vint>&dp,string &s,int k){
    int n=s.length();
    dp=vector<vint>(n+2,vint(k+1,0));
...
void solve(){
    ...
    For(i,1,n){
        For(j,i,n){
            ...
            int z=0;
            if((k-x)>=0){
                z=max(dp1[i-1][k-x],dp2[n+1-(j+1)][k-x]);
                l1[j-i+1]=max(z,l1[j-i+1]);
            }
            
        }
    }
    l1[0]=max(dp1[n][k],dp2[1][k]);
