...
void solve(){
    ...
    ll mx=*max_element(all(zoro));
    ll mn=*min_element(all(zoro));

    if((zoro[0]==mx  and zoro[3]==mn) or (zoro[0]==mn  and zoro[3]==mx)){
        yess;
        return;
    }
    if((zoro[1]==mx  and zoro[2]==mn) or (zoro[1]==mn  and zoro[2]==mx)){
        yess;
        return;
    }
    noo;
