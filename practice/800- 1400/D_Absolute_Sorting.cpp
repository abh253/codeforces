...
bool isdec(vll &v,int idx){
    for(int i=idx;i<v.size()-1;i++){
        if(v[i]<v[i+1])return false;
    }
    return true;
}
bool isinc(vll &v,int idx){
    for(int i=idx;i<v.size()-1;i++){
        if(v[i]>v[i+1])return false;
    }
    return true;
...
void solve(){
    ...
    if(mp.size()<=2){
        ...
        if((zoro[0]+zoro.back())%2==0){
            cout<<(zoro[0]+zoro.back())/2<<endl;
        }else mone;
    }else{
        mone;
        return;
...
//there are two cases
// if there are only two numbers with any frequency and their difference is even then can be made to a non decreasing
// otherwise if array is decreasing
// if half is inc and half is dec 
// 