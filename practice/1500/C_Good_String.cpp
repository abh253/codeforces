...
ll solve(vint &a,vint &b){
    if(a.size()==0 or b.size()==0)return max(a.size(),b.size());
    if(b[0]<a[0])return solve(b,a);//a[0]<b[0]
...
int main(){
	...
	while(testcase--){
        ...
        ll len=LONG_LONG_MIN;
        For(i,0,9){
            For(j,i,9){
                if(i==j){
                    len=max(len,(ll)v[i].size());
                }else{
                    len=max(len,solve(v[i],v[j]));
                }
            }
        }
