...
void process(int r,int c,vector<string>&zoro,vector<string>&v){

    FOR(i,k+1){
        if(r-i>=0 and c+i<m and c-i>=0){
            if(v[r-i][c+i]=='.' 
            or v[r-i][c-i]=='.')
            return;
        }
    ...
    for(int i=0;i<=60;i++){
        if(!(r-i>=0 and c+i<m and c-i>=0))return;

        if(v[r-i][c+i]=='.' or v[r-i][c-i]=='.')return;
...
void solve(){
    ...
    // FOR(i,n)cout<<zoro[i]<<endl;
    
    FOR(i,n){
        FOR(j,m){
            if(zoro[i][j]!=v[i][j]){noo;return;}
        }
    }
    yess;
