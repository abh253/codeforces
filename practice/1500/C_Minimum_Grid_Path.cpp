...
int main(){
	...
	while(testcase--){
        ...
        For(i,2,n-1){
            if(i%2==0){
                horzsum+=v[i];
                horzmin=min(v[i],horzmin);
                lashor=(horzsum-horzmin)+(n-hor)*horzmin;
                ans=min(ans,lashor+lasver);
                hor++;

            }else{
                vertsum+=v[i];
                vertmin=min(v[i],vertmin);
                lasver=(vertsum-vertmin)+(n-ver)*vertmin;
                ans=min(ans,lasver+lashor);
                ver++;
            }
        }
        cout<<ans<<endl;
