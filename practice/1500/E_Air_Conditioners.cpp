...
int main(){
	...
	while(testcase--){
		ll n,k;cin>>n>>k;
        vll v(n,LONG_LONG_MAX-400000);
        vll right(n,LONG_LONG_MAX-400000);
        vll left(n,LONG_LONG_MAX-400000);
    
        vll idx(k);vinput(idx);
        FOR(i,k){
            ll x;cin>>x;
            v[idx[i]-1]=x;
        }
        ...
        For(i,0,n-1){
            ct++;
            ct=min(ct,v[i]);
            right[i]=min(right[i],ct);
        }
        ct=v.back();
        rep(i,n-1,0){
            ct++;
            ct=min(ct,v[i]);
            left[i]=min(left[i],ct);
        }
        For(i,0,n-1){
