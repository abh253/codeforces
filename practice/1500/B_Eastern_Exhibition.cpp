...
int main(){
	...
	while(testcase--){
        ...
        um<ll,um<ll,bool>>mpx;
        um<ll,um<ll,bool>>mpy;
        ...
        FOR(i,n){
            ...
            ll disty=0;
            FOR(j,n){
                distx+=abs(v[i][0]-v[j][0]);
                disty+=abs(v[i][1]-v[j][1]);   
            }
            mpx[distx][v[i][0]]=1;
            mpy[disty][v[i][1]]=1;
            minx=min(minx,distx); 
        ...
        pll a={-1,-1};
        pll b={-1,-1};
        for(auto it:mpx[minx]){
            if(a.ff==-1)a.ff=it.ff;
            else a.ss=it.ff;
        }
        for(auto it:mpy[miny]){
            if(b.ff==-1)b.ff=it.ff;
            else b.ss=it.ff;
        }
        ll ansx=mpx[minx].size();
        ll ansy=mpy[miny].size();
        if(a.ss!=-1){
            ansx+=abs(a.ff-a.ss)-1;
        }
        if(b.ss!=-1){
            ansy+=abs(b.ff-b.ss)-1;
        }
