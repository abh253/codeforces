...
void solve(){
    ...
    unordered_map<int,vll>factonum;
    ...
    while(!q.empty()){
        ...
        FOR(i,fac[temp-1].size()){
            if(!visitedfac.count(fac[temp-1][i])){
                visitedfac[fac[temp-1][i]]=1;
                for(auto it:factonum[fac[temp-1][i]]){
                    if(!visited[it]){
                        q.push(it);
                        parent[it]=temp;
                        visited[it]=1;
