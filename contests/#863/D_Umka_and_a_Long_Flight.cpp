...
void solve(ll n,ll x1,ll y1,ll x2,ll y2,ll x,ll y){
    if(x1==x2 and y1==y2){
        if(x==x1 and y==y1)ans=true;
        return ;
    }
    ...
    if(y2-y1+1==fib[n]){
        ll nx=x1+fib[n-1]-1;
        ll ny=y1+fib[n]-1;
        if(check(x,y,x1,y1,nx,ny)){
            solve(n-1,x1,y1,nx,ny,x,y);
            return;
        }
        nx=x2-fib[n-1]+1;
        ny=y2-fib[n]+1;
        if(check(x,y,nx,ny,x2,y2)){
            solve(n-1,nx,ny,x2,y2,x,y);
            return;
        }
    }else if(x2-x1+1==fib[n]){
        ll nx=x1+fib[n]-1;
        ll ny=y1+fib[n-1]-1;
        if(check(x,y,x1,y1,nx,ny)){
            solve(n-1,x1,y1,nx,ny,x,y);
...
// every time there are two possibilities
// i need to do this