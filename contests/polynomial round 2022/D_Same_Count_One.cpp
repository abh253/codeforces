...
void solve(){
    ...
    if(onecnt%n!=0){
        ...
        for(int i=0;i<m;i++){
            ...
            // FOR(x,zoro.size()){
            //     cout<<zoro[x][0]<<" "<<zoro[x][1]<<endl;
            // }

            ll l=0,r=n-1;
            while(l<r){
                while(l<r and one[zoro[l][1]]>=nnd){
                    l++;
                }
                while(r>l and one[zoro[r][1]]<=nnd){
                    r--;
                }
                // cout<<l<<"dfd "<<r<<endl;
                if(l<r and v[zoro[l][1]][i]==0 and v[zoro[r][1]][i]==1){
                    ans.pb({zoro[l][1]+1,zoro[r][1]+1,i+1});
                    swap(v[zoro[l][1]][i],v[zoro[r][1]][i]);
                    one[zoro[l][1]]++;
                    one[zoro[r][1]]--;
                }
                l++,r--;
