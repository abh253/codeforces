...
ll n,d;
void kthancestor(int i,int p,vector<vll>&t,vll&stk,vll &a,vll&b){// a ka ancestor b m update krnege
    stk.pb(i);
    for(auto j:t[i]){
        if(j!=p){
            kthancestor(j,i,t,stk,a,b);
        }
    }
    ll siz=stk.size()-1;
    if(siz>d and a[i]){
...
ll dfs(int i,int p,vector<vll>&t,vll &a){
    ll ans=0;
    for(auto j:t[i]){
        if(j!=p){
            ll tempans=ans;
...
void solve(){
    ...
    vll stk;
    kthancestor(0,-1,t,stk,a,b);
    kthancestor(0,-1,t,stk,b,a);
    // errA(a);
    // errA(b);
    ll ans=0;
    ans+=dfs(0,-1,t,a);
...
// ancestor problem 
// aa- array need to be visited by a
// bb - array need to be visited by b
// every d ancestor of bb have to be visited by a
//