...
int main(){
	...
	while(testcase--){
		ll n;cin>>n;
        vector<string>v(n);
        um<string,int>mp;// will store the substring of size 3
		um<string,int>robin;//will store the ss of size 2
        FOR(i,n){
            cin>>v[i];
        }
        bool flag=0;
        FOR(i,n){
            if(mp.count(v[i]) or v[i][0]==v[i].back()){
                flag=1;
                break;
            }
			if(v[i].size()==2 and robin.count(v[i])){
				flag=1;
				break;
			}
            if(v[i].size()==3 and robin.count(v[i].substr(1))){
                flag=1;
                break;
            }
