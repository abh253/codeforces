...
ll findWeight(ll i,ll p,vector<vll>&t){
    ll ans=1;
    for(auto j:t[i]){
        if(j!=p){
            ans+=findWeight(j,i,t);
        }
    }
    return ans;
}

ll findCentral(ll n,vector<vll>&t){

    
    FOR(i,n){
        set<int>st;
        for(auto j:t[i]){
            st.insert(findWeight(j,i,t));
        }
        if(t[i].size()!=1 and st.size()==1){
            return i;
        }
    }
    return n-1;
