    ...
    if(k==0){
        ...
        for(int x=0;x<(1<<6);x++){
            if(__builtin_popcountll(x)!=0){
                ...
                // err2(ex,nex);
                ll x=power(2,ex,mod)-1;
                x+=mod;
                x%=mod;

                ll y=power(2,nex,mod);
                ll z=(x*y)%mod;
...
// why this solutio was wrong
// because i am taking a reference number which has k set bit and calculation solution around it
// example
// i am taking 1 1 0 , to caculate number of subsequence which will have subs. and with equal to this number
// but that is not always the case because 111 and 1110 can also have same and which is not included in my solution 


 // what about dp soluiton
 // lets do dp solution 