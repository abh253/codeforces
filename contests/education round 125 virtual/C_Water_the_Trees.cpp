...
ll solve2(ll mxele,vll&v){
    ll n=v.size();
    ll low=0;
    ll high=mxele*n-accumulate(all(v),0LL);
    high*=2;
    ll ans=high;
...
void solve(){
    ...
    ll mxele=*max_element(all(v));
    cout<<min(solve2(mxele,v),solve2(mxele+1,v))<<endl;
