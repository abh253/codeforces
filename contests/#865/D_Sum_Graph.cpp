...
void solve(){
    ...
    for(int i=0;i<a.size();i++){
        if(a[i]!=0)zoro.pb(a[i]);
    }
    list<ll> ans1;
    list<ll> ans2;

    if(n%2==1){
        ans1.push_front(zoro[0]);
        ans2.push_front(zoro[n-1]);
    }
    ...
    for(int i=(n%2);i<n;i++){
        if(!dir){
            ans1.push_front(zoro[i]);
            ans2.push_front(zoro[n-1-i]);
        }else{
            ans1.push_back(zoro[i]);
            ans2.push_back(zoro[n-1-i]);
        }
        dir=!dir;
    }
