...
ll helper(ll i,ll j, ll k,string &s){
    if(i==s.length()){
        return 0;
    }
    if(dp[i][j][k]!=-1){
    ...
    // i need maximum
    ll ans=INT_MIN;
    if(k==0){// i have not done the operation yet
        for(ll p=0;p<5;p++){
            // if(p==s[i]-'A')continue;
            ll sans=0;
            if(p<j){
                sans=helper(i+1,max(p,j),1,s)-value[p];
            }else{
                sans=helper(i+1,max(p,j),1,s)+value[p];
            }
            ans=max(sans,ans);
    ...
    ll curj=s[i]-'A';
    if(curj<j){
        ans=max(-value[curj]+helper(i+1,j,k,s),ans);
    }else{
        ans=max(value[curj]+helper(i+1,curj,k,s),ans);
    }
    return dp[i][j][k]=ans;
}
void solve(){
    ...
    string s;cin>>s;
    ll n=s.length();
    // reverse the string and do a recursive +memoization
    // we will call two functions one where we will change the currrent in all 5 possible way 
    // ans other not doint any change to the current digit
    // dp[i][j][k]=at ith index maximum till now if j and k(0/1)=>0 means no change yet and 1 means changed already

    dp=vector<vector<vll>>(n+1,vector<vll>(5,vll(2,-1)));
    reverse(all(s));

    

    ll ans=helper(0,0,0,s);
    cout<<ans<<endl;
