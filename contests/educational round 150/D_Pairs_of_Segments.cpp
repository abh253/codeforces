...
ll helper(ll i,ll j,ll k,vector<vll>&v){
    if(i==v.size()){
        if(k==0){
            return 0;
        }
    ...
    ll ans=v.size();
    if(k==0){
        // i can or can't start a segment
        
        if(v[i][0]>j){
            ans=min(ans,helper(i+1,max(j,v[i][1]),1,v));
        }
        ans=min(ans,1+helper(i+1,j,k,v));

    }else{
        
        if(v[i][0]<=j){
            ans=min(ans,helper(i+1,max(j,v[i][1]),0,v));
        }
        ans=min(ans,1+helper(i+1,j,k,v));
...
void solve(){
    ...
    ll n;cin>>n;
    // vll v(n);vinput(v);
    // n is in range to do n^2
    // okay 
    // approach:
    // i think sorting the segments and after it doing a recursive + memoization 
    // where dp[i][j][k] (if k==0 , then need to choose the start of new seg)
    //                  // will keep a number to tell end of last seg (j) there will n(order) different j

    //                    if(k==1), then need to choose the end of the already started segment
    //                      // will keep a number to tell the end of the seg
    //                      if till the end of the array segment pair not formed that be invalid so , return n
    //
    // dp.clear();                          
    dp=vector<vector<map<ll,ll>>>(n,vector<map<ll,ll>>(2));

    vector<vll>v(n,vll(2));
    FOR(i,n){
        cin>>v[i][0]>>v[i][1];
    }
    sort(all(v),sortLogic);

    ll ans=helper(0,-1,0,v);
    cout<<ans<<endl;
