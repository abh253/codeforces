...
for(int i=0;i<n;i++){
    if(!(b[i]==a[i] or ((b[i]<=b[(i+1)%n]+1) and b[i]>=a[i]))){
        noo;
        return;
    }
}
...
//i can make a equal to b if form non decreasing part of b corresponding a elements are smaller and 
//non decreasing element next to last of this series should be not less than last element of series of b-1
//i can try to react a from b
//