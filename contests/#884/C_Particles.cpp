...
void solve(){
    ...
    ll n;cin>>n;
    vll v(n);vinput(v);
    // i would like to negate the effect from negative particles
    // i can remove elements from the right or left its o
    // if all negative then i can choose maximum of those negatives
    // otherwise we can divide array in negative set and positive set if negative set has odd
    // length then it can be completely removed 
    // if it has positive length then i can harm minimum

    bool allneg=1;
    for(auto x:v){if(x>0)allneg=0;}
    ...
    while(even.size()>0 and even.back()<0){even.pop_back();}
    while(odd.size()>0 and odd.back()<0){odd.pop_back();}

    ll evensum=accumulate(all(even),0LL);
    ll oddsum=accumulate(all(odd),0LL);
