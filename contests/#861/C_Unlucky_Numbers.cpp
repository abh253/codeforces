...
bool dp[22][11][11][2][2];
ll curmn;
string curAns;
void solvesh(string &beg,string &lim,string &cur,int i,ll mn,ll mx,int isGreaterl,int isLesserR){
    if(i==beg.length()){
        if((mx-mn)<curmn){
            // cout<<cur<<endl;
            curmn=mx-mn,curAns=cur;
        }
        
        return;
    }
    if(dp[i][mn][mx][isGreaterl][isLesserR]){
        return;
    }
    ll si=beg[i]-'0',ei=lim[i]-'0';
    if(isGreaterl)si=0;
    if(isLesserR)ei=9;
    ...
    For(j,si,ei){
        ...
        cur[i]='0'+j;
        // err2(i,j);
        solvesh(beg,lim,cur,i+1,min(mn,j),max(mx,j),j>(beg[i]-'0'),j<(lim[i]-'0'));
    }
    dp[i][mn][mx][isGreaterl][isLesserR]=1;
}
void solve(){ 
    ...
    string beg=to_string(l);
    string lim=to_string(r);

    if(beg.length()!=lim.length()){
        FOR(i,beg.size()){
            cout<<"9";
        }
        nl;
