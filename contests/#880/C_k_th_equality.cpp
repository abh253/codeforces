...
void solve(){
    ...
    ll A,B,C,k;inp4(A,B,C,k);

    // we just need to set A and B , C is just their sum 
    ll mx=max(A,B);
    ...
    FOR(i,B)ub*=10;
    ub--;
    ll cmin=power(10,C-1,mod);
    ll cmax=power(10,C,mod)-1;
    // err2(cmin,cmax);
    ...
    For(i,la,ua){
        ll jmax=cmax-i;
        ll jmin=cmin-i;
        // jmin and jmax has to satisfy the digit count constraint
        if(jmax<lb or ub<jmin){
            continue;
        }
        jmin=max(jmin,lb);
        jmax=min(jmax,ub);
        ll total=jmax-jmin+1;
        if(total<k){
            k-=total;
        }else{
            cout<<i<<" + "<<jmin+k-1<<" = "<<i+jmin+k-1<<endl;
            return;
        }
    }
    mone;
