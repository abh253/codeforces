...
void solve(){
    ...
    // i can binary search on the ans 
    // and for r<g/2(can be given to every astrophysicist which can be saved)
    // after that we have to take care of the left and see if we can distribute in a way that we can save it
    // 
    ll save=0;
    ll x=(g+1)/2;
    x--;
    ...
    // i can give every number from 1 to g extra coins which will be saved
    // some will be given as set of g and other will be given as extra less than g/2 which can be saved
    ...
    while(l<r){
        ll mid=(l+r)/2;
        ll curleft=totalcoins-mid*g;

        ll mxcoin=(curleft+n-1)/n;
        if(mxcoin>x){
            l=mid+1;
        }else{
            r=mid;
