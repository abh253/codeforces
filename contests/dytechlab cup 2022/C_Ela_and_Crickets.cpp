...
bool check2(ll x,ll y,int n){
    if((x==2 or x==n-1) and (y==n-1 or y==2))return 1;
    return 0;

}
// bool check(int n,vector<vll>&c){
...
//     return false;
// }
bool logic(vll &a,vll &b){
    if(a[0]==b[0])return a[1]<b[1];
    return a[0]<b[0];
}
pair<bool,pll> checkvertex(ll n,vector<vll>&v){
    for(int i=0;i<v.size();i++){
        ll x=v[i][0];
        ll y=v[i][1];
        if((x==1 or x==n) and (y==1 or y==n))return {1,{x,y}};
    }
    return {0,{0,0}};
}
pll findcenter(vector<vll>&v){
    ll a=v[0][0]==v[1][0]?v[0][0]:v[2][0];
    ll b=v[0][1]==v[1][1]?v[0][1]:v[2][1];
    return {a,b};
}
void solve(){
    
    ll n;cin>>n;
    vector<vll>c(3,vll(2));
    FOR(i,3){
        cin>>c[i][0]>>c[i][1];
    }
    // sort(all(c),logic);
    ll tx,ty;cin>>tx>>ty;
    pll cen=findcenter(c);
    // auto aa=checkvertex(n,c);
    // cout<<cen.ff<<cen.ss<<endl;
    if((cen.ff==1 or cen.ff==n) and (cen.ss==1 or cen.ss==n)){
        
        if((tx==cen.ff) or (ty==cen.ss))yess;
        else noo;
        // cout<<tx<<ty<<endl;
