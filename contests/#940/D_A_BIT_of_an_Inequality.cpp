...
void solve(){
	...
	For(i,1,n){
		For(j,0,29){
			int t=!!(v[i-1]&(1<<j));
			For(k,0,1){
				prefix[i][j][k]+=(k==t)+prefix[i-1][j][k^t];
			}
		}
	}
	rep(i,n,1){
		For(j,0,29){
			int t=!!(v[i-1]&(1<<j));
			For(k,0,1){
				suffix[i][j][k]+=(k==t)+suffix[i+1][j][k^t];
			}
		}
	}
	...
	For(i,1,n){
		int z = 31 - __builtin_clz(v[i-1]);
		// +1 to take care of empty subbaray case
		ans+=1LL*(prefix[i-1][z][0]+1)*suffix[i+1][z][1];
		ans+=1LL*(prefix[i-1][z][1])*(suffix[i+1][z][0]+1);
	}
    cout<<ans<<endl;
