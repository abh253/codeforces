...
bool check(ll current,ll l,ll mid,vll&prefix){

    if(current<(prefix[mid]-prefix[l-1]))return true;
    return false;
}
vll helper(vll& v){
    ll n=v.size()-1;
    // we look at the slime at right which can eat it
    vll ans(n+1,INT_MAX);
    vll unique(n+1,INT_MAX);
    rep(i,n-1,1){
        if(v[i]==v[i+1]){
            unique[i]=unique[i+1];
        }else{
    ...
    For(i,1,n-1){
        l=i+1;
        r=n;
        while(r<n and v[r+1]>v[r])r++;

        ll lc=l;
        ll rc=r;
        // to see at which index the slime would be big enough to engulf v[i]
        while(lc<rc){
            ll mid=(lc+rc)/2;

            if(check(v[i],l,mid,prefix)){
                rc=mid;
            }else{
                lc=mid+1;
            }
        }
        if(check(v[i],l,lc,prefix)){
            if(v[i+1]<=v[i]){
                if(unique[i+1]!=INT_MAX){
                    ans[i]=max(unique[i+1]-i,lc-i);
                }
            }
            else
            ans[i]=lc-i;
