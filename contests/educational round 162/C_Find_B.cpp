...
ll build(int v,int tl,int tr,vll &t,vll &a){
    ...
    return t[v]=min(build(2*v+1,tl,tm,t,a),build(2*v+2,tm+1,tr,t,a));
...
ll sum(int v,int l,int r,int tl,int tr,vll &t){
    ...
    return min(sum(2*v+1,l,min(mid,r),tl,mid,t),sum(2*v+2,max(mid+1,l),r,mid+1,tr,t));
}
...
void solve(){
    ...
    FOR(i,q){
        ll l,r;
        cin>>l>>r;
        // ll mn=sum(0,l,r,1,n,t);
        ll sum=prefix[r]-prefix[l-1];
        ll one=onecnt[r]-onecnt[l-1];


        if(one){
            ll sumdone=(one-1)*2+(r-l+1-one);
            if(sum-sumdone>1){
                yess;
            }else{
                noo;
