...
ll helper(ll idx,ll curk,ll k,vll &b,vll &c,vector<vint>&dp){
    ...
    ll ans=0;
    if(tt+curk<=k){
        ans=max(c[idx]+helper(idx+1,curk+tt,k,b,c,dp),ans);
    }
        ans=max(helper(idx+1,curk,k,b,c,dp),ans);

    return dp[idx][curk]=ans;
}
...
int main(){
	...
	while(testcase--){
        ...
        vector<vector<int>>dp(n+1,vector<int>(30000,-1));
        ll ans=helper(0,0,k,bve,cve,dp);
        cout<<ans<<endl;
