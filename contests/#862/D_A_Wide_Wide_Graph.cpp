...
// two dfs needed
vector<vll> dp;
void dfs1(int i,int p,vector<vll>&t){
    // ll mx1=0,mx2=0;
    for(auto j:t[i]){
        if(j!=p){
            dfs1(j,i,t);
            if(dp[j][0]+1>dp[i][0]){
                dp[i][1]=dp[i][0];
                dp[i][0]=dp[j][0]+1;
            }else if(dp[j][0]+1>dp[i][1]){
                dp[i][1]=1+dp[j][0];
            }
        }
    }
}
void dfs2(int i,int p,vector<vll>&t){
...
void solve(){
    ...
    ll curidx=0;
    For(i,1,n){
        while(curidx<n and v[curidx]<i)curidx++;
        cout<<curidx+(n-curidx>0)<<" ";
    }
    nl;
