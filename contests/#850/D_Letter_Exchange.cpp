...
void solve(){
    ...
    // vll v(n);vinput(v);
    // w=0,i=1,n=2;
    vector<vector<vint>>v(3,vector<vint>(3));
    FOR(i,m){
        string s;cin>>s;
        ll a=0;
        ll b=0;
    ...
    For(i,0,2){
        For(j,i+1,2){
            ...
            while(v[i][j].size()>0 and v[j][i].size()>0){
                ans.pb({v[i][j].back(),j,v[j][i].back(),i});
                v[i][j].pop_back();
                v[j][i].pop_back();
            }
        }
    ...
    if(v[0][1].size()){
        ...
        while(v[0][1].size()>0 and v[2][0].size()>0){
              ans.pb({v[0][1].back(),1,v[2][0].back(),0});
                v[2][1].pb(v[2][0].back());
                v[0][1].pop_back();
                v[2][0].pop_back();
        ...
        while(v[1][2].size()>0 and v[2][1].size()>0){
            ans.pb({v[1][2].back(),2,v[2][1].back(),1});
            v[1][2].pop_back();
            v[2][1].pop_back();
        }

    }else{
         while(v[1][0].size()>0 and v[2][1].size()>0){
              ans.pb({v[1][0].back(),0,v[2][1].back(),1});
                v[2][0].pb(v[2][1].back());
                v[1][0].pop_back();
                v[2][1].pop_back();
        ...
        while(v[0][2].size()>0 and v[2][0].size()>0){
            ans.pb({v[0][2].back(),2,v[2][0].back(),0});
            v[0][2].pop_back();
            v[2][0].pop_back();
        }
    }
    ...
    mp[2]='n';
    FOR(i,ans.size()){
        err4(ans[i][0]+1,mp[ans[i][1]],ans[i][2]+1,mp[ans[i][3]]);
    }
