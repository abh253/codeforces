...
// }
string ans;
void answer(string &a,string&b,string &curans,int i, int j,int ast){
    if(i==a.length() or j==b.length()){
        if(j!=b.length() or i!=a.length()){
            curans+='*';
            ast++;
        }
        if(ast<=(curans.length()-ast)){
            ans=curans;
        }
        if(j!=b.length() or i!=a.length()){
            curans.pop_back();
    ...
    if(a[i]==b[j]){
            curans+=a[i];
            answer(a,b,curans,i+1,j+1,ast);
            curans.pop_back();
    }
    
    if(curans.empty() or curans.back()!='*'){
        curans.pb('*');
            answer(a,b,curans,i,j+1,ast+1);
            answer(a,b,curans,i+1,j,ast+1);
        curans.pop_back();
    }else{
        // curans.pb('*');
            answer(a,b,curans,i,j+1,ast);
            answer(a,b,curans,i+1,j,ast);
    // curans.pop_back();
    }
