...
void solve(){
    ...
    ll n,q;cin>>n>>q;
    imp=vll(n);vinput(imp);

    t=vector<vll>(n);
    zoro=vector<vll>(n,vll(2));   
    parent=vll(n);
    ...
    FOR(i,q){
        ...
        if(qtype==1){
            ...
            if(!mp[x].empty()){
                ...
                mp[p].insert({-zoro[hsi][0],hsi});
                mp[hsi].insert({-zoro[x][0],x});
            }
        }
    }
    // errA(parent);   
...
// observation
// in this problem i should
// use map with sets
// that would be suffiecient
// i need to keep track of parents and also maintain importance and size of every subtree