...
void solve(){
    ll n=2;
    ll m;cin>>m;
    vector<vll>v(3,vll(m+1));
    For(i,1,n){
        For(j,1,m)cin>>v[i][j];
    }


    vector<vll>suff_max_minus(3,vll(m+5,INT_MIN));
    vector<vll>suff_max_plus(3,vll(m+5,INT_MIN));

    For(i,1,2){
        rep(j,m,1){
            suff_max_minus[i][j]=max(suff_max_minus[i][j+1],v[i][j]-j);
            suff_max_plus[i][j]=max(suff_max_plus[i][j+1],v[i][j]+j);
        }
    }
    ...
    int r=1,c=1;
    ll ans=2*m-1;
    ans=max(ans,suff_max_minus[1][c+1]+2*m+1);
    ans=max(ans,suff_max_plus[2][c]);

    ll curtime=0;
    for(int moves=1;c<=m;moves++){
        if(moves%2==0){
            c++;
            curtime=max(curtime+1,v[r][c]+1);

            ll curr=curtime+2*(m-c)+1;
            if(c==m){

                curr=max(curtime+1,v[(r==1)?2:1][m]+1);

            }
            else if(r==1){
                
                curr=max(curr,suff_max_minus[1][c+1]+2*m-c+2);
                curr=max(curr,suff_max_plus[2][c]+1-c);


            }else{
                
                curr=max(curr,suff_max_minus[2][c+1]+2*m-c+2);
                curr=max(curr,suff_max_plus[1][c]+1-c);
            }
