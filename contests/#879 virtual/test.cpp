...
int dp[10][20];
bool populate(int i,int csum,int target,vector<int>&v){
    if(csum==target){
        return dp[i][csum]=1;
    }
    if(csum>target or i==v.size()){
        return dp[i][csum]=0;
    }
    ...
    cans=cans or populate(i,csum+v[i],target,v);

    cans=cans or populate(i+1,csum,target,v);

    return dp[i][csum]=cans;
}
