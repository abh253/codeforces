...
//j =if any digit is made greater than L[i]
// k if any digit made lower than r[i] 
int dp[100][2][2][2][2];
ll helper(ll i,ll lj,ll lk,ll rj,ll rk,string &L ,string &R){
    if(i==L.length()){
        return 0;
    }
    if(dp[i][lj][lk][rj][rk]!=-1){
        return dp[i][lj][lk][rj][rk];
    }
    ll curL=L[i]-'0';
    ll curR=R[i]-'0';
    ...
    ll sans=0;
    For(x,xlimit1,xlimit2){
        ll newlj=lj|(x>curL);
        ll newrj=rj|(x<curR);
        For(y,ylimit1,ylimit2){
            ll newlk=lk|(y>curL);
            ll newrk=rk|(y<curR);
            sans=max(sans,abs(x-y)+helper(i+1,newlj,newrj,newlk,newrk,L,R));
        }
    }
    return dp[i][lj][lk][rj][rk]=sans;
...
void solve(){
    ...
    string L,R;
    cin>>L>>R;
    // digit dp easy solution or else
    reverse(all(L));
    reverse(all(R));
    while(L.length()<R.length())L.pb('0');
    while(L.length()>R.length())R.pb('0');
    reverse(all(L));
    reverse(all(R));
    ll n=L.length();
    // err2(L,R);
