...
ll unequal(string &s,string &t,bool rev=0){
    
    if(rev==1)reverse(all(t));

    ll unequalcnt=0;
    FOR(i,s.length()){
        if(s[i]!=t[i])unequalcnt++;
    }
    if(rev==1)reverse(all(t));
...
void solve(){
    ...
    // vll v(n);vinput(v);
    string s,t;cin>>s>>t;
    // alice want to minimize the moves
    // bob wants to maximize the moves
    // bob move is fixed if reverse of the string is equal to the reverse of the 
    // i can go with greeedy approach
    // if i know which move is this then i can also know who is making the move and what are the orientaion of the strings, there are only two orientation - one where any one string is reversed other where no stirng is reversed

    ll unequalcnt=unequal(s,t,false);
    ll revunequalcnt=unequal(s,t,true);
    ...
    while(1){
        ...
        if(mid%2==0){
            if(mid%4==0){
                flag=(changes>=unequalcnt);              
            }else{
                flag=(changes>=revunequalcnt);
            }
        }else{
            if((mid-1)%4==0){
                flag=(changes>=unequalcnt);              
