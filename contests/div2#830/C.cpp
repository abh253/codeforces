...
void solve(){
    ...
    ll n,qq;cin>>n>>qq;
    vll v(n);vinput(v);
    vector<vll>query(qq,vll(2));
    FOR(i,qq){
        cin>>query[i][0]>>query[i][1];
    }
    ll sum=accumulate(all(v),0LL);
    ll xxor=0;
    for(int i=0;i<v.size();i++)xxor^=v[i];
    ll mx=sum-xxor;
...
// not clear about how we are finding boundries
// i know maximum value is mx;
// and there is no way i find a value greater than this
//the wrong is if i can remove some numbers from start and some from end there are many possiblities
// some from start and some from end
//for every right boundry where i am having value equal to mx i can check the left boundary
