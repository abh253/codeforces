...
int xxor(int v, int tl, int tr, int l, int r) {
    if (l > r) 
        return 0;
    if (l == tl && r == tr) {
        return t[v];
    ...
    int tm = (tl + tr) / 2;
    return xxor(v*2, tl, tm, l, min(r, tm))
           ^ xxor(v*2+1, tm+1, tr, max(l, tm+1), r);
}
void solve(){
    ...
    for(int i=0;i<q;i++){
        ll L=query[i][0];
        ll R=query[i][1];
        ll sasum=pre[R]-pre[L];
        ll saxor=xxor(1,0,n-1,L-1,R-1);
        ll sans=sasum-saxor;
        if(!amp.count(sans)){
            amp[sans]={L,R};
        }else{
