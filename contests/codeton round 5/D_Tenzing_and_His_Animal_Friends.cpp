...
void solve(){
    ...
    while(1){
        ...
        // now we need to subtract this from the edges
        string s(n,'0');
        for(auto x:active)s[x]='1';
        ans.pb(s);
        time.pb(mnwt);
        ...
        for(auto x:newactive)active.insert(x);
        // cout<<t[0][n-1]<<endl;
        // cout<<"n present"<<" ";
        // cout<<active.count(n-1)<<endl;
        if(active.count(n-1)){
            break;
        }
    }

    ll totaltime=accumulate(all(time),0LL);

    err2(totaltime,ans.size());
    FOR(i,ans.size()){
        err2(ans[i],time[i]);
    }
