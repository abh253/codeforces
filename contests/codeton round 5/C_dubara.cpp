...
void solve(){
    ...
    ll n;cin>>n;
    vll v(n);vinput(v);
    // if dp1[i]==-1 that means this type of number not seen yet
    // it will store the index of that maximum ans giving number
    ll ans=0;
    map<ll,set<ll>>mp;
    mp[v[0]].insert(0);
    ...
    For(i,1,n-1){
        ...
        if(mp.count(v[i])){
            ...
            ll sa=i+1+x;
            dp2[i]=max(dp2[i],sa);
            mp[v[i]].insert(dp2[i-1]-i);
        }
        else{
            mp[v[i]].insert(dp2[i]-i);
        }  
        ans=max(ans,dp2[i]);
    }
    cout<<ans<<endl;
