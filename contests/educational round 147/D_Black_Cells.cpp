...
// using binary search if i somehow get the 
// min no. of segments that will make k achievable
// okay i can do a pq system
// it will search until mid;
// and tell us the min segment it will take to achieve that k
// 
// binary search approach not giving proper way to do the problem
// 
// other approach 
// observations:
// it's always benificial to take segments which have size greater than or equal to 2
// here is the algorithm :
// seperate the segment of length 1 and length >1 
//  keep prefix sum
// prefix sum of number of 1s and other as number of length 1 segments 
// till that index


// sudo code
//make an array wher i keep the size of the segment 
//then keep the count of single segments and after when the total size of segments(>1) becomes greater than k
// break out
// after that do prefix sum 
// and if zoro.back()<k then no ans
// otherwise
// let's call one segment count -c and others ans -s;
// starting with last index in prefix
// calculate the cost to make k (if there is need to use 1seg only then use it)
// otherwise no need 
// and keep track of min ans
// output the min ans
// s>k then ll x=s-k => rn=ri-x this will be the point till where we need to travel
// calculating current= rn+2*(i-c);
// otherwise ri+2*((i-c)+(k-s)) 
// if it  is valid