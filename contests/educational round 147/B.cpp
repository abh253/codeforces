...
pll check(vll&a,vector<pll>&zoro){
    ll us=-1;
    ll s=-1;
    pll usp={-1,-1};
    pll sp={-1,-1};
    ...
    FOR(i,zoro.size()){
        ll as=1;
        // err2(zoro[i].ff,zoro[i].ss);
        For(j,zoro[i].ff+1,zoro[i].ss){
            if(a[j]<a[j-1]){
                as=0;
            }
        }
        ll siz=zoro[i].ss-zoro[i].ff;
        if(as){
            if(siz>s){
                s=siz;
                sp=zoro[i];
...
void solve(){
    ...
    // i need to find longest subarray which is sorted in b
    ll l;
    vector<pll>zoro;
    l=0;
    For(i,1,n-1){
