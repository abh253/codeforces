...
void solve(){
    ...
    ll n,m,q;cin>>n>>m>>q;
    string s;cin>>s;
    vector<vll>interval(m,vll(2));
    FOR(i,m){
        inp2(interval[i][0],interval[i][1]);
    }
    vll v(n);
    FOR(i,n){
        if(s[i]=='1')v[i]=1;
    ...
    ll zs=zoro.size();
    vll t(zs*4);
    build(0,0,zs-1,t,zoro);
    // cout<<sum(0,0,zs-1,0,zs-1,t)<<endl;
  
    ll onecnt=accumulate(all(v),0LL);
    FOR(j,q){
        ll x;cin>>x;
        x--;
        if(v[x]==1){
            ...
            onecnt--;
            if(newindex[x]!=-1){
                remove(0,newindex[x],0,0,zs-1,t);
            }
        }else{
            v[x]=1;
            onecnt++;
            if(newindex[x]!=-1){
                remove(0,newindex[x],1,0,zs-1,t);
            }
        }
        ...
        // cout<<t[0]<<endl;;
        ll r=min(onecnt-1,zs-1);

        ll zoroocnt=sum(0,0,r,0,zs-1,t);
        cout<<(r+1)-zoroocnt<<endl;
