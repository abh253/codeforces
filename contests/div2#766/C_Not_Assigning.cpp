...
void popWeights(int idx,int parent,vector<vector<int>>&ver,unordered_map<int,unordered_map<int,int>>&mp,int last){
	...
	for(auto it:ver[idx]){
		...
		if(last==2){
			mp[idx][it]=3;
			mp[it][idx]=3;
			popWeights(it,idx,ver,mp,3);
			last=3;
		}else{
			mp[idx][it]=2;
			mp[it][idx]=2;
			popWeights(it,idx,ver,mp,2);
		}
	}
}
...
int main(){
	...
	while(testcase--){
		...
		popWeights(0,-1,ver,mp,2);

		FOR(i,edges.size()){
			ans.push_back(mp[edges[i].ff][edges[i].ss]);
		}	
