...
void dfs(ll i,vll&t,vll&visited,set<ll>&vis,vll&comp){
    if(visited[i])return;
    comp.pb(i);
...
void solve(){
    ...
    // every component must have a cycle of size k
    vll visited(n+1);
    For(i,1,n){
        if(!visited[i]){
            set<ll>vis;
            vll comp;
            dfs(i,t,visited,vis,comp);
            // errA(comp);

            if(comp.size()>vis.size()){
                // means there is a cycle
                // just need the cycle to be of size k
                reverse(all(comp));
                while(comp.back()!=comp[0]){
                    comp.pop_back();
                }
                if((comp.size()-1)!=k){
                    noo;
