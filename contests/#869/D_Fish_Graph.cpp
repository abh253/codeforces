...
bool dfs(ll i,ll p,ll des,vll &visited,vll&cycle,set<ll>&cycleset){
    // if(des==i and cycle.size()!=0){
    //     cycle.pb(i);
    //     return 1;
    // }
    visited[i]=1;
    ...
    for(auto j:g[i]){
        if(j!=p){
            if(j==des){
                // and atleast two edges of j should not be present in the cycle
                // or if i am sure that i have the smallest cycle
                // furthermore i know in which direction i have to move
                cycle.pb(j);
                // cycleset.insert(j);
                return 1;
            }
            if(!visited[j]){
                if(dfs(j,i,des,visited,cycle,cycleset)){
                    ans=true;break;
                }
            }
        }
...
bool solveHelper(ll i){
    if(g[i].size()<4)return 0;
    // cout<<g[i].size()<<endl;
    vll visited(n);
    vll cycle;
    set<ll>cycleset;
    
    if(dfs(i,-1,i,visited,cycle,cycleset)){
        // errA(cycle);
        set<pll>edges;

        for(int i=0;i<cycle.size()-1;i++){
            edges.insert({cycle[i],cycle[i+1]});
        }
        ll cnt=0;
        vector<vll>nedges;
        // cout<<i<<endl;
        for(auto j:g[i]){
            if(!(edges.count({i,j}) or edges.count({j,i}))){
                nedges.pb({i,j});
                cnt++;
            }
            if(cnt==2)break;
        ...
        // FOR(x,2){
        //     err2(nedges[x][0],nedges[x][1]);
        // }
        if(cycleset.count(nedges[0][1]) or cycleset.count(nedges[1][1])){
            // cout<<"hello"<<endl;
            edges.clear();
            ll si=1,ei=cycle.size()-2;
            edges.insert({i,cycle[si]});
            edges.insert({i,cycle[ei]});
            while(si<ei and (cycle[si]!=nedges[0][1]) and (cycle[si]!=nedges[1][1])){
                si++;
            }
            ll oei=ei;
            while(ei>si and (cycle[ei]!=nedges[0][1]) and (cycle[ei]!=nedges[1][1])){
                ei--;
            }
            if(si==ei){
                ei=oei;
            ...
            if(!cycleset.count(nedges[0][1])){
                edges.insert({i,nedges[0][1]});
            }
            if(!cycleset.count(nedges[1][1])){
                edges.insert({i,nedges[1][1]});
            }
            // err2(si,ei);
                while(si<ei){
                    edges.insert({cycle[si],cycle[si+1]});
                    si++;
                }

        }else{
            edges.insert({nedges[0][0],nedges[0][1]});
            edges.insert({nedges[1][0],nedges[1][1]});
        }
...
// observations
// i will dfs the cycle and then
//  when i find a cycle // then i need to check if that 
// try from every node 
// and i this node has at least 4 edges then
// dfs to check if  this is a part of cycle
// if it is then i put out the answer
// if i don't find the answer after taking every index as u 
// then i terminate 
// ans output noo
