...
void solve(){
    ...
    for(int i=0;i<zoro.size();i++){
        ll rg=min(zoro[i].ff/zoro[0].ff,zoro.back().ff/zoro[i].ff);
        // cout<<rg<<endl;
        For(j,1,rg){
            
            if(!mp.count(zoro[i].ff*j) or zoro[i].ff%j!=0)continue;

            ll x=zoro[i].ff/j;
            ll y=zoro[i].ff*j;
            if(j==1){
                ll a=max(zoro[i].ss,0LL);
                ll b=max(zoro[i].ss-1,0LL);
                ll c=max(zoro[i].ss-2,0LL);
                

                ans+=(a*b*c);
            }else{
                if(mp.count(x) and mp.count(y))
                ans+=zoro[i].ss*mp[x]*mp[y];
