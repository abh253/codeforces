...
void solve(){
    ...
    while(l<n and r<n){
        ...
        while(r<n and fm.size()<m){
             for(ll j=1;j<=sqrt(v[r]);j++){
                if(v[r]%j==0){
                    ll a=j;
                    ll b=v[r]/j;
        ...
        while(l<n and fm.size()==m){
            flag=1;
            ans=min(v[r-1]-v[l],ans);

            for(ll j=1;j<=sqrt(v[l]);j++){
              if(v[l]%j==0){
                 ll a=j;
                 ll b=v[l]/j;
                if(a<=m){
                   fm[a]--;
                    if(fm[a]==0)fm.erase(a);
                }
                 if(b<=m){
                    fm[b]--;
                    if(fm[b]==0)fm.erase(b);
                 }
              }
            }
            l++;
