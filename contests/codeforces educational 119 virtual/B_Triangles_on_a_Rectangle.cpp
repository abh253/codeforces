...
// int calculate(ll bs,vll &a,vll &b,vll&c,ll h){

//     ll area=0;

//     for(int i=0;i<a.size();i++){
//         area=max(area,bs*a[i]);
//     }
//     for(int i=0;i<b.size();i++)area=max(area,bs*b[i]);
    
//     area=max(area,bs*height)
...
void solve(){
    ...
    ll k;cin>>k;vll x1(k);vinput(x1);
    cin>>k;vll x2(k);vinput(x2);
    cin>>k;vll y1(k);vinput(y1);
    cin>>k;vll y2(k);vinput(y2);

    ll area=0;
    area=max((x1.back()-x1[0])*h,area);
    area=max((x2.back()-x2[0])*h,area);
    area=max((y1.back()-y1[0])*w,area);
    area=max((y2.back()-y2[0])*w,area);
