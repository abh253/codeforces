...
void solve(){
    ...
    ll ans=0;
    // if none of the fixed is placed first

    pos.pb(m);

    ll sans=min(m,type1+pos.size()-2);
    ans=max(ans,sans);

    For(i,1,pos.size()-2){

        ll cur=pos[i];
        ll lpeople=type1+i-1;
        ll rpeople=pos.size()-(i+2)+type2;

        ll lspace=cur-1;
        ll rspace=m-cur;

        ll sans=1+min(lpeople,lspace)+min(rspace,rpeople);

        ans=max(ans,sans);
    }
    sans=min(m,type2+pos.size()-2);
    ans=max(ans,sans);

    // ans=max(ans,min(m,type1+type2));
...
// observations
// the person with fixed seat in mind should be accounted for first
// because they wont sit any other seat

// algorithm
// i will put all the x in a array 
// ans start with smallest index
// and all position on the left can be taken by type 2 and all the index smaller than cur
// and right side will be index greater than current and type 1 
// if this number is greater than the seats on the right
// and there are seats left on the left of the cur position then cur will not be given a
// seat instead the extra from the right will
// and same in case of if left have extra people and less seat but right part have some seat left
