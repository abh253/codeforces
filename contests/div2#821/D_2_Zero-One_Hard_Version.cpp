...
// ll solve(int i,string &a,string &b){



    
// }
//the change will be done in pairs 
//if at two position strings are not equal either we can do operation direct using y amount
//or we can include all the other letters in between to use x many times
...
void solve(){
    ...
    if(x>=y){
        ...
        // dp[2]=min(dp[0]+x*(pos[2]-pos[1]),dp[0]+y);
        for(int i=2;i<ucnt;i++){

            if(i%2==1)
            dp[i]=min(dp[i-2]+(pos[i]-pos[i-1])*x,dp[i-1]+y);

            else
            dp[i]=min(dp[i-2]+(pos[i]-pos[i-1])*x,dp[i-1]);// wait we can also leave this index unused and dp[i-2]+y;
...
// i need to check for all possibilities
// like doing non-adjacent and adjacent operations
// for changing two adjacent we can do one x operation or 2 y operations
// when we are performing y operations we can either do 1 y operation 
// (j-i)  x operations to change both of them
