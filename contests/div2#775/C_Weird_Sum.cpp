...
int main(){
	...
	while(testcase--){
		ll n,m;cin>>n>>m;
        vector<vector<int>>mpx(100001);
        vector<vector<int>>mpy(100001);
        FOR(i,n){
            FOR(j,m){
                ll x;cin>>x;
                mpx[x].pb(i);
        ...
        ll ans=0;
        For(i,1,100000){
            if(mpx[i].size()<2)continue;

            sort(all(mpx[i]));
            ll cnt=0;
            ll last=0;
            for(int j=mpx[i].size()-2;j>=0;j--){
                ll cur=(mpx[i][j+1]-mpx[i][j])+last+cnt*(mpx[i][j+1]-mpx[i][j]);
                ans+=cur;
                last=cur;
                cnt++;
            }
        ...
        For(i,1,100000){
            if(mpy[i].size()<2)continue;

            sort(all(mpy[i]));
            ll cnt=0;
            ll last=0;
            for(int j=mpy[i].size()-2;j>=0;j--){
                ll cur=(mpy[i][j+1]-mpy[i][j])+last+cnt*(mpy[i][j+1]-mpy[i][j]);
                ans+=cur;
                last=cur;
                cnt++;
            }
