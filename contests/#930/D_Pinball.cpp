...
void solve(){
    ...
    for(int i=1;i<=n;i++){
        if(s[i]=='>'){
            pos[0].pb(i);
            prefix[0].pb(prefix[0].back()+i);
        }else{
            pos[1].pb(i);
            prefix[1].pb(prefix[1].back()+i);
        }
    }

    auto sum=[&](ll x,ll l,ll r){
        if(r<l)return 0LL;

        return prefix[x][r]-(l>0?prefix[x][l-1]:0);
    };
    vll ans;
    For(i,1,n){
        ll li=lower_bound(all(pos[0]),i)-pos[0].begin()-1;
        ll ri=upper_bound(all(pos[1]),i)-pos[1].begin();
        ll lc=li;
        ll rc=pos[1].size()-ri;
        ll res;
        // err2(li,ri);
        if(s[i]=='<'){
            if(rc<lc){
                // finishes on right
               res=-2*sum(0,li-rc,li)+2*sum(1,ri,pos[1].size()-1)+n+i+1;
            }else{
                // finishes on left
               res=-2*sum(0,1,li)+2*sum(1,ri,ri+lc-1)+i;
            }
        }else{
            if(lc<rc){
                // finishes on left

                res=-2*sum(0,1,li)+2*sum(1,ri,ri+lc)-i;

            }else{
                // ends on right
                // problem
                // err2(li,ri);
                res=-2*sum(0,li-rc+1,li)+2*sum(1,ri,pos[1].size()-1)+n+1-i;
            }
        }
        // cout<<res<<endl;
        ans.pb(res);
