...
void solve(){
    ...
    ll bn=INT_MAX;
    for(int i=0;i<n;i++){
        azero=min(azero,abs(a[0]-b[i]));
        an=min(an,abs(a[n-1]-b[i]));
        bzero=min(bzero,abs(b[0]-a[i]));
        bn=min(bn,abs(b[n-1]-a[i]));
    }
    ll ans=0;
    if(azero+bzero<=abs(a[0]-b[0])){
        ans+=azero+bzero;
    }else ans+=abs(a[0]-b[0]);

    if(bn+an<=abs(b[n-1]-a[n-1])){
        ans+=bn+an;
    }else ans+=abs(b[n-1]-a[n-1]);

    ll ans2=0;
    if(bzero+an<=abs(b[0]-a[n-1])){
        ans2+=bzero+an;
    }else ans2+=abs(b[0]-a[n-1]);

    if(azero+bn<=abs(a[0]-b[n-1])){
        ans2+=azero+bn;
    }else ans2+=abs(a[0]-b[n-1]);
