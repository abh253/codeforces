...
vll dir={0,1,0,-1,0};
pll dfs(ll x,ll y){
    if(!mp.count({x,y}))return {x,y};
    if(answer.count({x,y}))return answer[{x,y}];
    visited[{x,y}]=1;    

    pll ans={20000000,2000000};
    for(int i=0;i<4;i++){
        ll p=x+dir[i];
        ll q=y+dir[i+1];
        if(!visited.count({p,q})){
            pll z=dfs(p,q);
            if(abs(z.ff-x)+abs(z.ss-y)<abs(ans.ff-x)+abs(ans.ss-y)){
                ans=z;
            }
        }
...
void solve(){
    ...
    for(int i=0;i<n;i++){
        if(!answer.count({v[i][0],v[i][1]})){
            dfs(v[i][0],v[i][1]);
        }
    }

    for(int i=0;i<n;i++){
        pll x=answer[{v[i][0],v[i][1]}];
        cout<<x.ff<<" "<<x.ss<<endl;
    }
