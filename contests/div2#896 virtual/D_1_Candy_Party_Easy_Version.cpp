...
void solve(){
    ...
    FOR(i,n){
        ...
        if(num!=0){
            // to check if the number is in the form ...000111110000...
            ll z=num;
            while((z&1)!=1){
                z/=2;
            }
            ll msb=__lg(z);
            ll allone=(1LL<<(msb+1))-1;//...00011111111111
            // if num & allone==num then we are certain of the structure
            if((allone&z)!=allone){
                noo;
                return;
            }
            ll y=__builtin_ctzll(num);// returns the trailing zero
            ll x=__builtin_popcountll(num)+y;
