...
void solve(){
    ...
    ll ans=abs(v[b-1][0]-v[a-1][0])+abs(v[b-1][1]-v[a-1][1]);
    if(a>k){
        // in this case the path inbetween could be made of big cities

        ll amin=ans;
        ll bmin=ans;
        FOR(i,k){
            amin=min(amin,abs(v[a-1][0]-v[i][0])+abs(v[a-1][1]-v[i][1]));
            bmin=min(amin,abs(v[b-1][0]-v[i][0])+abs(v[b-1][1]-v[i][1]));
        }
        ll cans=amin+bmin;
        cout<<min(ans,cans)<<endl;
        return;
    }
    // only case when a is big city and b is not.
    FOR(i,k){
        // find minimum distance from any big city to b
        ll sans=abs(v[b-1][0]-v[i][0])+abs(v[b-1][1]-v[i][1]);
        ans=min(ans,sans);
    }
    cout<<ans<<endl;
