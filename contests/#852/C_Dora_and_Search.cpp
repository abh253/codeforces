...
pll build(int v,int tl,int tr,vector<pll> &t,vll &a){
    if(tl==tr){
        t[v].ff=a[tl];
        t[v].ss=a[tl];
        return t[v];
    ...
    ll tm=(tl+tr)/2;
    build(2*v+1,tl,tm,t,a);
    build(2*v+2,tm+1,tr,t,a);
    t[v].ff=min(t[2*v+1].ff,t[2*v+2].ff);
    t[v].ss=max(t[2*v+1].ss,t[2*v+2].ss);

    return t[v];
}
pll sum(int v,int l,int r,int tl,int tr,vector<pll> &t){
    ...
    pll a=sum(2*v+1,l,min(mid,r),tl,mid,t);
    
    pll b=sum(2*v+2,max(mid+1,l),r,mid+1,tr,t);
...
void solve(){
    ...
    FOR(i,n){
        ...
        while(l<=r){
            ...
            pll x=sum(0,i,mid,0,n-1,t);
            // err2(i,mid);
            // err2(x.ff,x.ss);
            if(v[i]!=x.ff and v[i]!=x.ss and v[mid]!=x.ff and v[mid]!=x.ss){
                err2(i+1,mid+1);
                return;
            }else{
                l=mid+1;
