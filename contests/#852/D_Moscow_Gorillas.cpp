...
void solve(){
    ...
    ll ans=1;
    ll l=min(pos1[1],pos2[1]);
    ll r=max(pos1[1],pos2[1]);

    ans+=sum(l)+sum(r-l-1)+sum(n-1-r);

    For(i,2,n){
        ll ln=min(pos1[i],pos2[i]);
        ll rn=max(pos1[i],pos2[i]);

        if(!((rn<=r and  rn>=l) or (ln<=r and  ln>=l))){
            if(rn<l){
                ans+=(n-r)*(l-rn);
            }else if(ln>r){
                ans+=(ln-r)*(l+1);
