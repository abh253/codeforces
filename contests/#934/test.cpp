...
class hashString{
    ...
    hashString(string &s){
        ...
        pp[0]=1;
        for(int i=0;i<n;i++){
            if(i>0)pp[i]=(p*pp[i-1])%mod;
            ppi[i]=inverse(pp[i],mod);
            curhash=(curhash+(s[i]-'a'+1)*pp[i])%mod;
            prefix[i+1]=curhash;
        }
    }
    ll get(ll start,ll end){
        ...
        ll ans=(prefix[end+1]-prefix[start]+mod)%mod;
        ans= (ans*ppi[start])%mod;
        return ans;
    }
};
...
void solve(){
    ...
    string s="wuwjtdeeweyzzxwniflodvekzbzcjilzrmcrdiwo";
    string t="wuwjtdeeweyzzxwniflodvekzbzcjilzrmcrdiwo";
    reverse(all(t));

    hashString ss(s);
    hashString tt(t);

    err2(ss.get(0,s.length()-1),tt.get(0,t.length()-1));
    // err2(ss.get(3,4),tt.get(7,8));
