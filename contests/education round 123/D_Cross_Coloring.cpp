...
int main(){
	...
	cin>>testcase;
	while(testcase--){
		ll n,m,k,q;inp4(n,m,k,q);
        vector<vll>query(q,vll(2));
        set<ll>row;
        set<ll>col;
        ll points=0;
        FOR(i,q){
        ...
        rep(i,q-1,0){
            if(row.find(query[i][0])==row.end() and col.size()<m){
                points++;
                row.insert(query[i][0]);
                col.insert(query[i][1]);
            }else if(col.find(query[i][1])==col.end() and row.size()<n){
                points++;
                row.insert(query[i][0]);
                col.insert(query[i][1]);
            }
        ...
        cout<<power(k,points,998244353)<<endl;
