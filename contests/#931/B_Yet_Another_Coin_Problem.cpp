...
ll help2(ll i,ll n,vll &coin){
    if(n==0)return 0;
    if(i==coin.size())return 1e9;
    ll x=n/coin[i];

    ll ans=x+help2(i+1,n%coin[i],coin);
    
    if(x>0){
        ans=min(x-1+help2(i+1,n%coin[i]+coin[i],coin),ans);
    }
    return ans;
}
void solve(){
    ...
    ll n;cin>>n;
    vll coin={15,10,6,3,1};
    
    ll ans=help2(0,n,coin);
