...
int main(){
	...
	while(testcase--){
        ...
        FOR(i,n){
            ...
            ll x,y,z;
            cin>>x>>y>>z;
            coin[x]=max(coin[x],y*z);
        }
        
        For(i,1,C){
            for(ll j=2*i;j<=C;j+=i){
                coin[j]=max(coin[j],coin[i]*(j/i));
            }
        }
        For(i,1,C){
            coin[i]=max(coin[i],coin[i-1]);
        }
        ll m;cin>>m;
        vector<vll> monster(m,vll(2));
        vll ans(m,-1);
        
        FOR(i,m){
            cin>>monster[i][0]>>monster[i][1];

            auto x=upper_bound(all(coin),monster[i][0]*monster[i][1])-coin.begin();
            if(x<=C){
                ans[i]=x;
            }
        }
