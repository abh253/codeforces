...
int dfs(int i,int p,map<int,set<int>>&t,set<int>&visited){
    visited.insert(i);

    int current=1;
    for(auto j:t[i]){
        if(j==p or visited.count(j))continue;
        current+=dfs(j,i,t,visited);
    }
    return current;
}
int kSimilarity(string s1, string s2) {
    ...
    map<int,set<int>>t;
    for(int i=0;i<n;i++){
        a[i]=20*(s1[i]-'a')+count1[s1[i]-'a'];
        b[i]=20*(s2[i]-'a')+count2[s2[i]-'a'];
        t[a[i]].insert(b[i]);
        count1[s1[i]-'a']++;
        count2[s2[i]-'a']++;
    }
...
void solve(){
    ...
    string s1="abcbcaabcdefbabacdcdabcbcbcbc";
    string s2="ababccfabcdeababdcdcbcacbcbcb";
    kSimilarity(s1,s2);
