...
void solve(){
    ...
    // if we use binary search to iterate
    // for mid we will check the minimum days required to remove all numbers from 1 to mid
    // let say it's if x >k then r=mid-1; 
    // else if x<=k then l=mid so in the end i will have the maximum number(z) we can 1 to z in k days

    // how will we calculate x for every mid
    //  
    if(v[0]!=1){
        cout<<1<<endl;
        return;
