...
void solve(){
    ...
    ll g=a[0]*b[0];
    For(i,1,n-1){
        ll newlcm=lcm*b[i]/(__gcd(lcm,b[i]));
        ll newg=__gcd(g,a[i]*b[i]);
        if(newg%newlcm==0){
            lcm=newlcm;
            g=newg;
        }else{
...
// as we know we need bi*di == bi+1*di+1==di+1*di+1
// that actually means we want to take lcm of (bi,bi+1,bi+2)
//this will be the cost of one packet 
// to check if this cost is legal or not check __gcd(a[i]*b[i]...,a[n]*b[n])
