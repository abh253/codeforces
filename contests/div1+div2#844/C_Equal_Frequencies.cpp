...
void solve(){
    ...
    rep(j,mxl,1){
        ...
        vector<vll>chg(26,vll(26,0));

        vector<pair<ll,ll>>zoro;
        For(i,0,25){
            
            zoro.pb({C[i],i});
        }
        sort(zoro.begin(),zoro.end());
        // FOR(i,zoro.size()){
        //     cout<<zoro[i].ff<<" ";
        // }
        // nl;
        ll zs=zoro.size();
        // ll l=0,r=zoro.size()-1;
        ll r=zs-1;
        ll l=zs-1;
        while(l>=0 and zoro[l].ff>=cnt){
            l--;
        }
        while(l>=0 and r>l){
            ll need=cnt-zoro[l].ff;
            ll have=zoro[r].ff-cnt;
            if(need<have){
                zoro[l].ff+=need;
                zoro[r].ff-=need;
                chg[zoro[r].ss][zoro[l].ss]+=need;
                l--;
            }else if(need>have){
                zoro[l].ff+=have;
                zoro[r].ff-=have;
                chg[zoro[r].ss][zoro[l].ss]+=have;
                r--;
            }else{
                zoro[l].ff+=have;
                zoro[r].ff-=have;
        ...
        l=0;
        r=zs-1;
        while(r>=0 and zoro[r].ff>=cnt)r--;
        // cout<<l<<" "<<r<<endl;
        while(l<r){
            ll need=cnt-zoro[r].ff;
            ll have=zoro[l].ff;
            if(need<have){
                zoro[r].ff+=need;
                zoro[l].ff-=need;
                chg[zoro[l].ss][zoro[r].ss]+=need;
                r--;
            }else if(need>have){
                zoro[l].ff-=have;
                zoro[r].ff+=have;
                chg[zoro[l].ss][zoro[r].ss]+=have;
                l++;
            }else{
                zoro[l].ff-=have;
                zoro[r].ff+=have;
        ...
        FOR(i,n){
            ...
            while(idx[x]<26 and chg[x][idx[x]]==0)idx[x]++;
