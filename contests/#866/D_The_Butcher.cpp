...
bool check(ll n,ll area,map<ll,multiset<ll>>a,map<ll,multiset<ll>>b){
    ...
    for(int i=0;i<n;i++){
        if(!a[h].empty()){
            auto x=a[h].begin();
            w-=(*x);
            b[*x].erase(b[*x].find(h));
            a[h].erase(x);
        }
        else if(!b[w].empty()){
            auto x=b[w].begin();
            h-=(*x);
            a[*x].erase(a[*x].find(w));
            b[w].erase(x);
        }else {
            return 0;
        }
...
void solve(){
    ...
    if(area%h_max==0){
        if(check(n,area,a,b)){
            ans.insert({h_max,area/h_max});
        }
    }
    
    if(area%w_max==0){
        if(check(n,area,b,a)){
            ans.insert({area/w_max,w_max});
        }
    }
    cout<<ans.size()<<endl;
    for(auto it:ans){
...
// observation
// first cut gives alot of information
// if first cut was made horizontal then maximum width will be the final widht (similarly for height)
// and then 
// let's discuss for height h and w=total area /h
// but we need to check if it's a valid pair
// because their might be instance where width match
// eg {5,2}, {2,3},{1,5}

// so to check 
// if we have initial height
// 