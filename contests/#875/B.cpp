...
void solve(){
    ...
    map<ll,ll>ma;
    map<ll,ll>mb;
    // m store the maximum continuos length of every number present in the array
    ll ai=0;
    ll bi=0;
    for(int i=1;i<n;i++){
        if(a[i]!=a[i-1]){
            ma[a[i-1]]=max(ma[a[i-1]],i-ai);
            ai=i;
        }
        if(b[i]!=b[i-1]){
            mb[b[i-1]]=max(mb[b[i-1]],i-bi);
            bi=i;
        }
    }
    
    ma[a[n-1]]=max(n-ai,ma[a[n-1]]);
    mb[b[n-1]]=max(n-bi,mb[b[n-1]]);

    ll ans=0;
    for(auto it:ma){
        ans=max(ans,mb[it.ff]+it.ss);
    }
    for(auto it:mb){
        ans=max(ans,ma[it.ff]+it.ss);
    }
