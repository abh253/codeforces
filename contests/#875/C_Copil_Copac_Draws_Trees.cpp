...
ll ans;
map<pll,ll>order;
void dfs(ll i, ll p,ll cans,vector<vll>&t,set<ll>&sor){
    ans=max(ans,cans);
    pll pp={i,p};
    ...
    for(auto j:t[i]){
        if(j!=p){
            pll cp={i,j};
            if(cp.ff>cp.ss)swap(cp.ff,cp.ss);

            ll curorder=order[cp];
        
            auto x=sor.upper_bound(curorder);
            sor.insert(curorder);
            if(x!=sor.end() and porder>curorder){
                dfs(j,i,cans+1,t,sor);
            }else dfs(j,i,cans,t,sor);
            sor.erase(curorder);
        }
    }
}
