...
bool isCycle(ll i,vector<vll>&gp,vint & visited,vint& cstack){
    visited[i]=1;
    cstack[i]=1;

    for(auto j:gp[i]){
        if(cstack[j])return true;
        if(visited[j])continue;
        if(isCycle(j,gp,visited,cstack))return true;
    }
    cstack[i]=0;
    return false;
}

void topological(ll i,vector<vll>&gp,vint &visited,vll &tt){
    visited[i]=1;

    for(auto j:gp[i]){
        if(visited[j])continue;
        topological(j,gp,visited,tt);
    }
    tt.push_back(i);
}
void solve(){
    ...
    FOR(i,n){
        if(!visited[i] and isCycle(i,gp,visited,cstack)){
            mone;
            return;
        }
    }
