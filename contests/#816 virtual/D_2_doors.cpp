...
void solve(){
    ...
    vll fans(n);
    //every index is already set with the bits that can be on
    // we need lexiographically minimum
    // so we will start with 1 index and se the info given about it and 
    // try to minimize it
    // and do similarly with all other indexes.
 
    // every bit which is needed to be zero is zero
    // errA(ans);
    FOR(i,n){
       
         for(int k=0;k<30;k++){
             // i will check for every bit with the childs bit if kth bit of x is on 
             // and all kth bit of all the j numbers is on then this bit in i can stay off 
            ll bt=1;
            for(auto it:info[i]){
                ll j=it.ff,x=it.ss;
                ll xbit=((1LL<<k)&x)!=0;
                ll kbitj=((1LL<<k)&ans[j])!=0;
...
// observations
// ai | aj =x;
// means one at least one of them as same bits on as x
// and we neeed to make lexiographically smallest so, 
// we want to minimize lower index  
// okay do 1 traversal to see which index can have which open bits
// after that when doing 2nd traversal make larger index number larger
//