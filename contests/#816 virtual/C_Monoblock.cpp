...
void solve(){
    ...
    vll v(n);vinput(v);
    // key observation
    // every set of same numbers contribut 1 to every subarray it is present in
    // how will i store the awesomeness of the array 
    // after every change the awesomness is changing
    // need to take care of that
    // when a element is changing

    // at every query either awesomeness changes or stays the same
    

    // when a position where a[i]!=a[i+1] is present in an subarray that is contributing 1 to the awesomeness
    ...
    FOR(j,q){
        ...
        ll x;cin>>x;
        i--;
        if(i>0 and v[i]!=v[i-1])ans-=(i*(n-i));
        if(i<n-1 and v[i]!=v[i+1])ans-=(i+1)*(n-(i+1));
        v[i]=x;
        if(i>0 and v[i]!=v[i-1])ans+=(i*(n-i));
        if(i<n-1 and v[i]!=v[i+1])ans+=(i+1)*(n-(i+1));
...
// a set of identical number will always contribute 1 to all the subsequents 
//  it is present in    
//  