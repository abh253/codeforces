...
vector<pair<ll,ll>> helper(vll &a,ll m){
...
void solve(){
    ...
    vector<pair<ll,ll>>ax=helper(a,m);
    vector<pair<ll,ll>>bx=helper(b,m);
    if(ax.size()!=bx.size()){
        noo;
        return;
    }
    // for(int i=0;i<ax.size();i++){
    //     cout<<ax[i].ff<<" "<<ax[i].ss<<endl;
    //     cout<<bx[i].ff<<" "<<bx[i].ss<<endl;

    // }   
    for(int i=0;i<ax.size();i++){
        if(ax[i].ff!=bx[i].ff or ax[i].ss!=bx[i].ss){
            noo;
            return;
        }
    }
