...
void solve(){
    ...
    ll n;cin>>n;
    // vll v(n);vinput(v);
    // in every array i am concerned with two smallest element of an array
    // take minimum from all the array and put it in one of the array 
    vector<vll>v;
    ll minfirst=1e9;
    ll secMinSum=0;
    FOR(i,n){
        ll m;cin>>m;
        vll zoro(m);vinput(zoro);
        sort(all(zoro));
        v.pb({zoro[0],zoro[1]});
        secMinSum+=zoro[1];
        minfirst=min(minfirst,zoro[0]);
    }
    ll ans=0;
    for(int i=0;i<n;i++){
        ans=max(minfirst+secMinSum-v[i][1],ans);
    }
    cout<<ans<<endl;
