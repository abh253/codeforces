...
vector<int> z_function(string s) {
  ...
  // i am given the portals 
    // if i am at position xi then there can be cases where it can use more than one portal 
    // evertime chosing the portal which takes usop to futhest point is benificial
    // how to do it
    // at a point i need to know the furthest point i can go from that point
    // bi is a point of interest because, i can precompute futhest i can reach if i just keep goint to each portal's bi
    // now,
    // first for every every bi calculate if we are at bi what is the maximum distance we can travel
    // we can do this by finding the next closest bi and use its already calculate dist(some kind of dp);

    // save in map key=bi and value=li , ri
    // if bi is similar then make li=min and ri =max
void solve(){
    ...
    map<ll,pll>mp;
    for(int i=0;i<n;i++){
        ll l,r,a,b;inp4(l,r,a,b);
        if(!mp.count(b))
        mp[b]={l,r};
        else{
            mp[b].ff=min(l,mp[b].ff);
            mp[b].ss=max(r,mp[b].ff);
        }
    }
    vll bees;
    for(auto it:mp){
    ...
    ll beeslen=bees.size();
    vll dp(beeslen);
    FOR(i,beeslen){
        dp[i]=bees[i];
    }
    // dp[beeslen-1]=bees[beeslen-1];
    
    rep(i,beeslen-2,0){
        if(mp[bees[i+1]].ff<=bees[i] and bees[i]<=mp[bees[i+1]].ss){
            dp[i]=dp[i+1];
            mp[bees[i]].ff=min(mp[bees[i+1]].ff,mp[bees[i]].ff);
            mp[bees[i]].ss=max(mp[bees[i+1]].ss,mp[bees[i]].ss);
        }
    }
    ...
    FOR(i,q){
        ll x;cin>>x;
        auto y=lower_bound(all(bees),x)-bees.begin();

        if(y==beeslen){
            cout<<x<<" ";
        }else{
            if(x>=mp[bees[y]].ff and x<=mp[bees[y]].ss){
                cout<<dp[y]<<" ";
            }else{
                cout<<x<<" ";
            }
