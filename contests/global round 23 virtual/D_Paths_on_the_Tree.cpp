...
pll dfs2(int i,ll k,vector<vll>&t){

    ll c=t[i].size();
    if(c==0 ){
        return {k*value[i],(k+1)*value[i]};
    }
    ll kleft=k/c;
    ll kmod=k%c;
    ...
    for(auto j:t[i]){
        pll sans=dfs2(j,kleft,t);
        ans1+=sans.ff;
        diff.pb(sans.ss-sans.ff);
    }
...
void solve(){
    ...
    value=vll(n);vinput(value);
  
    ll ans=dfs2(0,k,t).ff;
    cout<<ans<<endl;
