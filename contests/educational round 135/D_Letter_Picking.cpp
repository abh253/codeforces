...
int help(int i,int j,string &s){
    ...
    int ans1=min((help(i+2,j,s)!=1?dp[i+2][j]:(s[i]<s[i+1]?2:(s[i]>s[i+1]?0:1))),
            help(i+1,j-1,s)!=1?dp[i+1][j-1]:(s[i]<s[j]?2:(s[i]>s[j]?0:1)));
      int ans2=min(help(i,j-2,s)!=1?dp[i][j-2]:(s[j]<s[j-1]?2:(s[j]>s[j-1]?0:1)),
            help(i+1,j-1,s)!=1?dp[i+1][j-1]:(s[j]<s[i]?2:(s[j]>s[i]?0:1)));
...
void solve(){
    ...
    string s;cin>>s;
    int n=s.length();
    dp=vector<vint>(n,vint(n,-1));

    int ans=help(0,n-1,s);
    if(ans==2){
        cout<<"Alice"<<endl;
    }else if(ans==0){
        cout<<"Bob"<<endl;
