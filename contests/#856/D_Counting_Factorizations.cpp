...
ll inverse(ll x,ll m){
    ...
    return inv[x]=power(fact[x],m-2,m);
}
...
void solve(){
    ...
    if(pp<n){
        ...
        ll dp[pp+1][n+1];
        inv=vll(2*n+5,-1);
        //dp[i][j]= number of ways after processing 
        ...
        For(i,1,pp){
            For(j,0,n){
                ...
                if(i>=j){
                    if(j>0)
                    dp[i][j]=((inverse(a[i-1]-1,mod2)*dp[i-1][j-1])%mod2+(inverse(a[i-1],mod2)*dp[i-1][j])%mod2)%mod2;
                    else
                    dp[i][j]=(inverse(a[i-1],mod2)*dp[i-1][j])%mod2;
                }
                // cout<<dp[i][j]<<" ";
            }
            // cout<<endl;
        ...
        ll ans=(fact[n]*dp[pp][n])%mod2;
        FOR(i,b.size()){
            // cout<<"hello"<<endl;
            ans=(ans*inverse(b[i],mod2))%mod2;
        }
