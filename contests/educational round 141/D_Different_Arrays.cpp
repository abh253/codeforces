...
void solve(){
    ...
    // state- dp[i][j]=> count of when ith number is j
    // number can be in range -90000 to 90000
    ...
    for(int i=1;i<n-1;i++){
        ...
        for(int j=0;j<=200000;j++){
            if(dpo[j]!=0){
                  dpn[v[i+1]+j]=(dpn[v[i+1]+(j-C)+C]+dpo[j])%mod2;

                if((j-C)!=0)
                dpn[v[i+1]-(j-C)+C]=(dpn[v[i+1]-(j-C) +C]+dpo[j])%mod2;
            }
