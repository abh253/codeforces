...
void solve(){
    ...
    // actually i should try both condition whe
    vector<pll>ans;
    if(pos>7 and neg>7){
        if(v[mxidx]>0){
            FOR(i,n){
        ...
        if(neg<=7 and pos>0){
            ll posidx=-1;
            FOR(i,n){if(v[i]>0)posidx=i;}
            FOR(i,5){
                ans.pb({posidx,posidx});
            }
            FOR(i,n){
            ...
            ll negidx=-1;
            FOR(i,n){if(v[i]<0)negidx=i;}
            FOR(i,5){
                ans.pb({negidx,negidx});
            }
            FOR(i,n){
