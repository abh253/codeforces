...
ll build(int v,int tl,int tr,vll &t,vll &a){
    ...
    return t[v]=max(build(2*v+1,tl,tm,t,a),build(2*v+2,tm+1,tr,t,a));
...
ll sum(int v,int l,int r,int tl,int tr,vll &t){
    ...
    return max(sum(2*v+1,l,min(mid,r),tl,mid,t),sum(2*v+2,max(mid+1,l),r,mid+1,tr,t));
}
...
void solve(){
    ...
    //at each unlocked card we either add it to victory point or use it for unlocking.
