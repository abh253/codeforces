...
// vector<vll>dp;
// set<pair<string,ll>>strcnt;
// ll helper(ll i,ll j,string &s,string &left,string &right,string &curs){
//     strcnt.insert({curs,j});
//     ll n=s.length();
//     ll m=left.length();
//     if(i==n or j==m){
//         return j==m;
...
//     if((s[i]-'0')>=(left[j]-'0') and (s[i]-'0')<=(right[j]-'0')){
//         curs.pb(s[i]);
//         if(!strcnt.count({curs,j})){
//             sans+=helper(i+1,j+1,s,left,right,curs);
//         }
//         curs.pop_back();
//     }
//     sans+=helper(i+1,j,s,left,right,curs);
//     return dp[i][j]=sans;
...
void solve(){
    ...
    // use dp to find the total bekarpass

    vector<vll>dp(n,vll(m));

    vll dp2(m+1); // it will keep the count of possible numbers at jth position which lie after current index
    ...
    ll totalpossible=1;
    FOR(i,m){
        ll diff=right[i]-left[i];
        totalpossible*=(diff+1);
    }
    // if i just sum 

    cout<<bekarpass<<endl;
    cout<<totalpossible<<endl;
    if(totalpossible>bekarpass){
        yess;
    }else noo;
