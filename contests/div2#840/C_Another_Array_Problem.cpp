...
void solve(){
    ...
    if(n>=4){
        ...
        if((v[2]>=v[0] and v[2]>=v[1]) or(v[0]>=v[1] and v[0]>=v[2])){
            cout<<3*max(v[0],v[2]);
            nl;
        }
        else{
            ...
            ans=max(ans,3*max(v[0],abs(v[1]-v[2])));
            ans=max(ans,3*max(v[2],abs(v[0]-v[1])));
...
// maximising sum of array

// for length 3 
// if maximum is in the middle
// 1 4 3 ->3 3 3 // only change first and second or second and third number

// 13 14 1 // change every number to first-last
