...
int main(){
	...
	cin>>testcase;
    ll ran=17;
    vector<vector<int>>v(power(2,ran,mod)+1,vector<int>(ran+1));
    for(int i=1;i<=power(2,ran,mod);i++){
        for(int j=ran;j>=0;j--){
            if((i&(1<<j))!=0){
                v[i][j]++;
            }
	...
	while(testcase--){
		ll l,r;cin>>l>>r;
        ll ans=0;
        vll arr(r-l+1);vinput(arr);

        vector<int>bit(ran+1);
        ...
        for(int i=0;i<=ran;i++){
            // cout<<v[r][i]-v[l>0?l-1:0][i]<<" ";

            if(bit[i]!=(v[r][i]-v[l>0?l-1:0][i])){        
                ans+=(1<<i);
            }
        }
        // nl;
