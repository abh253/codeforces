...
// i can't see how i can use bs
// observations
// that 
// idea - if i fix d=r-l such that d 
// not woring because ti will still go in n^2 
// is there any optimazation which i am not seeing

// D 
// I thought doing first put a[i]-I in the set then we can find best answer consist of two element
// after that I was unable to think how to approach further


// editorial i found online
//You can solve problem D even if instead of finding 3 best 
// elements we had to find k best elements in time complexity
//  of O(k*nlogn). For finding best two, you can do the 
// following : For all i, first insert a[i]-i in the set. 
// Now iterate on i from left to right, erase a[i]-i from set
//  and find maximum value in set(mx) and set b[i] = a[i]+mx+i. 
// b[i] now contains the answer for maximum 2 values. 
// Now you repeat this by inserting b[i]-i and setting as c[i] = a[i]+mx+i again. 
//  Repeating this allows to pick k best elements in O(k*nlogn)