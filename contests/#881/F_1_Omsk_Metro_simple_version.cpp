...
void dfs(ll i,ll p,map<ll,vll>&t,vll&value,vll&cur,ll mx=0,ll cursum=0){
    cursum+=value[i];
    if(cursum<0){
        cursum=0;
    }
...
void solve(){
    ...
    ll n=max(ll(t.size()),1LL);
    vll minval(n);
    vll maxval(n);
    dfs(0,-1,t,value,maxval);
    for(auto &x:value)x*=-1;
    dfs(0,-1,t,value,minval);
    for(auto &x:minval)x*=-1;
