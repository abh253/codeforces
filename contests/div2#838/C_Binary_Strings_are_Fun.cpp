...
void solve(){
    ...
    for(int i=1;i<n;i++){
        if(s[i]=='0'){
            ...
            ll zcnt=(onecnt[i]-zerocnt[i]+i+1)/2;
        
            // ll twooption=i-zcnt;

            ans+=power(2,i,mod2);
            if(zcnt-1>=0){ans-=power(2,zcnt-1,mod2);}
            // cout<<"zerocnt "<<zcnt<<endl;
            ans=(ans+mod2)%mod2;

        }else{
            ll ocnt=(zerocnt[i]-onecnt[i]+i+1)/2;

            // ll twooption=i-ocnt;
            // cout<<"ocnt "<<ocnt<<endl;
            
            ans+=power(2,i,mod2);
            if(ocnt-1>=0){ans-=power(2,ocnt-1,mod2);}
            ans=(ans+mod2)%mod2;
        }
        // cout<<ans<<endl;
    }
