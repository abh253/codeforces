...
int side;
bool solve(int i,int mask,vector<int>&v){
    ...
    int c=total/side;
    int rem=(c+1)*side-total;
    
    
    for(int j=0;j<v.size();j++){
        if(v[j]<=rem and (mask&(1<<j))==0){
            if(solve(i,mask|(1<<j),v)){
                ans=true;
                break;
            }
        }
    }
    cout<<i<<" "<<total<<" "<<" "<<mask<<endl;
    return dp[i][mask]=ans;
}
bool makesquare(vector<int>& ms) {
    int x=accumulate(ms.begin(),ms.end(),0);
    if(x%4)return 0;
    side=x/4;
    int n=ms.size();
    dp=vector<vector<int>>(4,vector<int>(1<<n,-1));
    return solve(0,0,ms);
